...
class lokasi_loo_json extends CI_Controller
{
	...
	function add()
	{
		...
		$reqTerletak= $this->input->post("reqTerletak");
		$reqLokasiGedung= $this->input->post("reqLokasiGedung");
		$reqDasarHukum= $this->input->post("reqDasarHukum");
		...
		$reqNamaCabang= $this->input->post("reqNamaCabang");
		...
		$set->setField("NAMA_CABANG", $reqNamaCabang);
		$set->setField("TERLETAK", setQuote($reqTerletak));
		$set->setField("LOKASI_GEDUNG", setQuote($reqLokasiGedung));
		$set->setField("DASAR_HUKUM", setQuote($reqDasarHukum));
		...
		$arrDataAttach= array();
		$data_attachement = new LokasiLoo();
		$data_attachement->selectByParamsAttachment(array("LOKASI_LOO_ID"=>$reqId), -1,-1, " AND COALESCE(NULLIF(A.VMODE, ''), NULL) IS NULL");
		$z=0;
		...
		$FILE_DIR = "uploadslokasiloo/".$reqId."/";
		makedirs($FILE_DIR);
		...
		$set_attachement->setField("LOKASI_LOO_ID", $reqId);
