...
class tanda_tangan_json extends CI_Controller {
	...
	function combo() 
	{
		$this->load->model("TandaTangan");
		$tanda_tangan = new TandaTangan();
		$tanda_tangan->selectByParams(array("NOT TANDA_TANGAN_ID" => "0"));
		$i = 0;
		while($tanda_tangan->nextRow())
		{
			$arr_json[$i]['id']		= $tanda_tangan->getField("TANDA_TANGAN_ID");
			$arr_json[$i]['text']	= $tanda_tangan->getField("NAMA");
			$i++;
		}
