...
class permohonan_stpd_json extends CI_Controller
{
	...
	function add()
	{
		...
		$reqTotalHari= $this->input->post("reqTotalHari");
		$reqStatusSurat= $this->input->post("reqStatusSurat");
		$reqSebelumStatusSurat= $this->input->post("reqSebelumStatusSurat");
		...
		$reqTotalRealisasi= str_replace('.','',$this->input->post("reqTotalRealisasi"));
		// echo $reqTotalRealisasi;exit;
		// $reqOrang= $this->input->post("reqOrang");
		...
		$set->setField("TANGGAL_BERANGKAT", dateTimeToDBCheck($reqTanggalBerangkat));
		$set->setField("TANGGAL_KEMBALI", dateTimeToDBCheck($reqTanggalKembali));
		...
		$set->setField("TOTAL_HARI", ValToNullDB($reqTotalHari));
		...
		$set->setField("PENGAJUAN_DISETUJUI_ID", $reqPengajuanDisetujuiId);
		...
		$arrstatusrealisasi= [];
		$arrstatusrealisasi= ["REVISI", "SETUJU", "SETUJUDRAFT", "SETUJUREVISI", "SETUJUKIRIM"];
		if(in_array($reqStatusSurat, $arrstatusrealisasi))
		{
			...
			// kecuali status sbelum kirim lewati, karena simpan data di akhir
			if($reqSebelumStatusSurat !== "KIRIM" || $reqStatusSurat == "REVISI")
			{
				...
				$vexe= getExtension($reqFileRealisasi['name']);
				// echo $vexe;exit;
				...
				$status->setField("TOTAL_REALISASI", ValToNullDB($reqTotalRealisasi));
				$status->setField("DOKUMEN_ACUAN", str_replace("'", "&quot;", $reqDokumenAcuan));
				$status->setField("LOKASI_DINAS", str_replace("'", "&quot;", $reqLokasiDinas));
				$status->setField("TANGGAL_BERANGKAT", dateTimeToDBCheck($reqTanggalBerangkat));
				$status->setField("TANGGAL_KEMBALI", dateTimeToDBCheck($reqTanggalKembali));

				if(!empty($vexe))
				{
					$this->load->model("PermohonanStpd");
					$renameFile = "DOC_REALISASI_" . date("Ymdhis") . rand() . "." . $vexe;
					...
					if($status->updatestatusrealisasipakaifile())
					{
						$reqSimpan=1;
					}
				}
				...
				{
					if($status->updatestatusrealisasitanpafile())
					{
						$reqSimpan=1;
					}
		...
		$arrstatusrealisasi= [];
		// bug 
		// $arrstatusrealisasi= ["SETUJU", "SETUJUDRAFT", "SETUJUREVISI"];
		$arrstatusrealisasi= ["SETUJUDRAFT", "SETUJUREVISI"];
		if
		(
			in_array($reqStatusSurat, $arrstatusrealisasi)
			||
			(
				($statussuratsebelum == "SETUJUDRAFT" || $statussuratsebelum == "SETUJUREVISI" || $statussuratsebelum == "SETUJU") 
		...
		)
		{
			...
			foreach ($reqPermohonanStpdBiayaDinasId as $key => $value) {
				if(empty($value))
				{
					...
					{
						// kl ada maka perlu di simpan
						// echo $reqAlokasi[$key]."xxx".$reqPengajuan[$key]."xxx".$reqRealisasi[$key]."xxx".$reqOrang[$key]."xxx".$reqKelompokId[$key];exit;
						$setinsert = new PermohonanStpd();
						$setinsert->setField("PERMOHONAN_STPD_ID", $reqId);
						$setinsert->setField("ALOKASI_BIAYA", $reqAlokasi[$key]);
						$setinsert->setField("PENGAJUAN_BIAYA", ValToNullDB(dotToNo($reqPengajuan[$key])));
						$setinsert->setField("REALISASI", ValToNullDB(dotToNo($reqRealisasi[$key])));
		...
		if
		(
			$reqStatusSurat == "DRAFT" || $reqStatusSurat == "REVISI" 
			|| ($reqStatusSurat == "KIRIM" && empty($vinfoid)) 
			|| ($reqStatusSurat == "KIRIM" && $reqSebelumStatusSurat == "DRAFT")
			// bug 
			|| ($reqStatusSurat == "SETUJU" && $reqSebelumStatusSurat == "KIRIM") 
			|| ($reqStatusSurat == "KIRIM" && $reqSebelumStatusSurat == "REVISI") 
		)
		{
			if(!empty($reqPengajuan))
			{
				...
				foreach ($reqPengajuan as $key => $value) {
					...
					$vkelompokid= $reqKelompokId[$key];

					$vrealisasi= $reqRealisasi[$key];
					if($reqStatusSurat == "SETUJU" && $reqSebelumStatusSurat == "KIRIM")
					{
						$vrealisasi= $value;
					}
					...
					$setinsert->setField("REALISASI", ValToNullDB(dotToNo($vrealisasi)));
					$setinsert->setField("KELOMPOK_ORANG", ValToNullDB($reqOrang[$key]));
					$setinsert->setField("KELOMPOK_ID", ValToNullDB($vkelompokid));
					if($setinsert->insertbiaya())
					{
						$reqSimpan=1;
					}
		...
		if($reqStatusSurat == "KIRIM" || $reqStatusSurat == "SETUJU"|| $reqStatusSurat == "SETUJUKIRIM"){
			/* GENERATE QRCODE */
			include_once("libraries/phpqrcode/qrlib.php");

			$FILE_DIR = "uploads/sppd/" . $reqId . "/";
			makedirs($FILE_DIR);
			$filename = $FILE_DIR . $reqStatusSurat . '.png';
			$errorCorrectionLevel = 'L';
			$matrixPointSize = 2;

			if($reqStatusSurat == "SETUJUKIRIM"){
				$kodeParaf=$reqNomor."-".date("dmYHis")."-".$this->NAMA;
			}
			else {
				$kodeParaf=date("dmYHis")."-".$this->NAMA;
			}
		...
		if ($reqSimpan==1 ) {
			...
			else
			{
				...
				else if($reqStatusSurat == "SETUJU")
				{
					...
					// bug untuk update total realisasi
					$status = new PermohonanStpd();
					$status->setField("PERMOHONAN_STPD_ID", $reqId);
					$status->setField("KET_REALISASI", $reqKeteranganRealisasi);
					$status->setField("DOKUMEN_ACUAN", str_replace("'", "&quot;", $reqDokumenAcuan));
					...
					$status->setField("TANGGAL_BERANGKAT", dateTimeToDBCheck($reqTanggalBerangkat));
					$status->setField("TANGGAL_KEMBALI", dateTimeToDBCheck($reqTanggalKembali));
					if($status->updatesetujui())
					{
					}
					// echo $status->query
					unset($status);

					// tutup karena nnti buat triger surat dua kali
					// $setdetil= new PermohonanStpd();
	...
	function teruskan()
	{
		...
		$set= new PermohonanStpd();
		$set->setField("PERMOHONAN_STPD_ID", $reqId);
		$set->setField("USER_BANTU_APPROVAL_NIP", $userid);
		if($set->updateuserbantuteruskan())
		{
			$inforeturninfo= "Naskah berhasil diteruskan.";
			...
			$slog->setField("INFORMASI", $this->JABATAN." (".$this->NAMA.")");
			$slog->setField("CATATAN", $reqInfoLog);
			$slog->setField("LAST_CREATE_USER", $userid);
			$slog->insertlog();
			unset($slog);
		}
		else
		{
			$inforeturninfo= "Naskah gagal diteruskan.";
		}
	...
	function logparaf()
	{
		...
		// khusus status draft atau revisi
		$arrstatusrealisasi= [];
		$arrstatusrealisasi= ["SETUJUPARAF"];
		if(in_array($reqStatusSurat, $arrstatusrealisasi))
		{
			$reqPermohonanStpdBiayaDinasId= $this->input->post("reqPermohonanStpdBiayaDinasId");
			$reqRealisasi= $this->input->post("reqRealisasi");
		...
		if(!empty($vexe))
		{
			...
			if($status->updaterealisasipakaifile())
			{
			}
		}
		else
		{
			if($status->updaterealisasitanpafile())
			{
			}
		}
		// echo $status->query;
		...
		$this->paraf_proses($reqId, "PARAF", $reqInfoLog,$reqNomor);
	}

	function paraf_proses($reqId, $reqSource, $reqInfoLog= "", $reqNomor)
	{
		$this->load->model("PermohonanStpd");
		$set = new PermohonanStpd();

		$satuankerjaganti= $this->SATUAN_KERJA_ID_ASAL;
		$kodeParaf = "PARAF" . $this->ID;
		...
		if ($set->paraf()) {
			...
			$errorCorrectionLevel = 'L';
			$matrixPointSize = 2;
			$kodeParaf2=$reqNomor."-".date("dmYHis")."-".$this->NAMA;

			QRcode::png($kodeParaf2, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
			// END OF GENERATE QRCODE
