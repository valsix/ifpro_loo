...
class trloi_json extends CI_Controller
{
	...
	function add()
	{
		...
		if($reqStatusData == "LAMPIRAN" && !empty($buttonlampiranupload))
		{
			...
			$FILE_DIR = "uploadsloo/".$reqTrLooId."/";
			makedirs($FILE_DIR);
			...
			$set_attachement->setField("TR_LOO_ID", $reqTrLooId);
			$set_attachement->setField("VMODE", $reqJenisTujuan);
			$set_attachement->deleteModeAttachment();
			$reqJenis = $reqJenisTujuan.generateZero($reqTrLooId, 10);
			$jumlahfile= 0;
		...
		if ($reqSimpan==1) 
		{
			...
			$arrDataAttach= array();
			$data_attachement = new TrLoi();
			$data_attachement->selectByParamsAttachment(array("TR_LOI_ID"=>$reqId), -1,-1, " AND COALESCE(NULLIF(A.VMODE, ''), NULL) IS NULL");
			$z=0;
			...
			if ( ($reqStatusData == "DRAFT" && empty($reqKondisiStatusData)) || ($reqStatusData == "PARAF" && $reqKondisiStatusData == "UBAHDATADRAFTPARAF") || ($reqStatusData == "DRAFT" && $reqKondisiStatusData == "UBAHDATAREVISI") )
			{
				...
				if(!empty($reqSatuanKerjaIdParaf))
				{
					...
					for ($i = 0; $i < count($reqSatuanKerjaIdParaf); $i++) {
						if ($reqSatuanKerjaIdParaf[$i] == "") {
							...
							$adaData= $setdetil->getCountByParams(array("TR_LOI_ID" => $reqId, "SATUAN_KERJA_ID_TUJUAN" => $reqSatuanKerjaIdParaf[$i]));
				...
				if(!empty($reqSatuanKerjaIdParaf))
				{
					$userbantu= new TrLoiParaf();
					$userbantu->setField("TR_LOI_ID", $reqId);
					$userbantu->deleteuserbantu();
					unset($userbantu);

					// reset ulang nomor
					$userbantu= new TrLoiParaf();
					$userbantu->selectByParams(array("TR_LOI_ID" => $reqId), -1, -1, " AND COALESCE(NULLIF(KONDISI_PARAF, ''), NULL) IS NULL", "ORDER BY NO_URUT ASC");
					$nomorurut=1;
					while($userbantu->nextRow())
					{
						$checkparaf= new TrLoiParaf();
					...
					{
						$checkparaf= new TrLoiParaf();
						$checkparaf->setField("TR_LOI_ID", $reqId);
				...
				$checkparaf= new TrLoiParaf();
				$checkparaf->selectByParams(array(), -1, -1, " AND A.STATUS_BANTU IS NULL AND KONDISI_PARAF = 'PARALEL' AND A.TR_LOI_ID = ".$reqId);
				$checkparaf->firstRow();
				$checkparafuserdireksi= $checkparaf->getField("NO_URUT");

				$checkparaf= new TrLoiParaf();
				$checkparaf->selectByParams(array(), -1, -1, " AND A.STATUS_BANTU = 1 AND KONDISI_PARAF = 'PARALEL' AND A.TR_LOI_ID = ".$reqId);
				$checkparaf->firstRow();
	...
	function paraf_proses($arrparam)
	{
		...
		if ($set->paraf()) {
			...
			{
				$set= new TrLoi();
				$statusSurat= $set->getStatusSurat(array("A.TR_LOI_ID" => $reqId));
