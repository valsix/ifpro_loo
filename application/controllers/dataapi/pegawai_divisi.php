...
class pegawai_divisi extends CI_Controller {
	...
	function index()
	{
		...
		if($obj->status == "200")
		{
			...
			else
			{
				...
				where a.departemen_id like '".$divisiid."%'
				order by a.departemen_id
				";*/
				$sql="
				select
				a.pegawai_id id, a.nama, a.jenis_kelamin jeniskelamin, a.email
				, sk.satuan_kerja_id jabatanid, sk.jabatan, a.satuan_kerja_id cabangid, cb.nama cabang
				, sk.kelompok_jabatan kode_jabatan
				from pegawai a
				left join
				(
				...
				inner join satuan_kerja_fix sk on a.pegawai_id = sk.nip
				where sk.satuan_kerja_id like '".$divisiid."%'
				order by sk.satuan_kerja_id
				";
				// echo $sql;exit;
				$infoquery= $this->db->query($sql)->result_array();
				$arrResult= $infoquery;
