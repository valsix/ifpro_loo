...
class pegawai_kelompok extends CI_Controller {
	...
	function index()
	{
		...
		if($obj->status == "200")
		{
			...
			if(empty($kelompokid))
			{
				...
				$arrResult["message"] = "Isikan Divisi id";
			}
			else
			{
				$sql="
				...
				(
					...
					(
						...
						from satuan_kerja_kelompok_group where satuan_kerja_kelompok_id = ".$kelompokid."
					) x where sk.kelompok_jabatan = x.kelompok_jabatan
				)
				and exists
				(
					select 1
					...
					(
						...
						from satuan_kerja_kelompok_detil where satuan_kerja_kelompok_id = ".$kelompokid."
					) x where a.satuan_kerja_id = x.satuan_kerja_id
				)
