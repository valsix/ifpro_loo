...
class approval_trace_json extends REST_Controller {
    ...
    function index_get() {
        ...
        if($reqStatusSurat == "AKAN_DISETUJUI")
        {
            $statement= " AND SM_INFO IN ('AKAN_DISETUJUI', 'NEXT_DISETUJUI') AND A.STATUS_SURAT IN ('PARAF', 'VALIDASI')";

            $sOrder = " ORDER BY INFO_STATUS_TANGGAL DESC";
            $surat_masuk->selectByParamsStatus(array(), -1, -1, $userMobile->ID, $statement, $sOrder);
        }
        else
        {
            //$statement= " AND ((A.USER_ATASAN_ID = '".$userMobile->ID."' AND  A.APPROVAL_DATE IS NULL) OR (A.USER_ATASAN_ID = '".$reqUserGroup.$userMobile->ID."' AND A.APPROVAL_DATE IS NOT NULL )) AND A.STATUS_SURAT IN ('VALIDASI', 'PARAF')";
            ...
            $sOrder = " ORDER BY INFO_STATUS_TANGGAL DESC";
            $surat_masuk->selectByParamsPersetujuan(array(), -1, -1, $userMobile->ID, $statement, $sOrder);
        }
