...
class absensi_manual_json extends CI_Controller {
	...
	function json() 
	{
		...
		$allRecord = $absensi_manual->getCountByParams(array(), $statement);
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $absensi_manual->getCountByParams(array(), $dsplyRange, $dsplyStart,  " AND (UPPER(B.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') ". $statement. $statement_privacy , " ORDER BY JAM DESC ");
