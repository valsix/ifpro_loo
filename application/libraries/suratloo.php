...
class suratmasukinfo
{
	...
	var $UMLAH_KOTAK_MASUK_SURAT_KEPUTUSAN_DIREKSI_TERBACA;
	...
	function infolinkdetil($jenisid)
	{
		...
		{
			$arrdata= array(
				"linkstatusdetil"=>"surat_pengantar_pengiriman_lihat"
				, "linkstatusedit"=>"surat_pengantar_pengiriman_add"
			);
		}
		else if($jenisid == "22")
		{
			$arrdata= array(
				"linkstatusdetil"=>"bon_permintaan_barang_lihat"
				, "linkstatusedit"=>"bon_permintaan_barang_add"
			);
		}
		else if($jenisid == "23")
		{
			$arrdata= array(
				"linkstatusdetil"=>"laporan_kerusakan_inventaris_lihat"
				, "linkstatusedit"=>"laporan_kerusakan_inventaris_add"
			);
		}
		else if($jenisid == "24")
		{
			$arrdata= array(
				"linkstatusdetil"=>"laporan_kerusakan_kendaraan_lihat"
				, "linkstatusedit"=>"laporan_kerusakan_kendaraan_add"
			);
		}
		return $arrdata;
	}
	...
	function getInfo($suratMasukId, $jenisSurat = "INTERNAL")
	{
		...
		if ($surat_masuk->firstRow()) {
			...
			$this->TEMBUSAN_PARAM = $surat_masuk->getField("TEMBUSAN_PARAM");
			$this->KEPADA_PARAM = $surat_masuk->getField("KEPADA_PARAM");
	...
	function getInfoAsc($suratMasukId, $jenisSurat = "INTERNAL")
	{
		...
		if ($surat_masuk->firstRow()) {
			...
			$this->EKSTERNAL_TEMBUSAN_ID = $surat_masuk->getField("EKSTERNAL_TEMBUSAN_ID");
			$this->EKSTERNAL_TEMBUSAN = $surat_masuk->getField("EKSTERNAL_TEMBUSAN");
			$this->NAMA_KAPAL = $surat_masuk->getField("NAMA_KAPAL");
			$this->PPB = $surat_masuk->getField("PPB");
			$this->JABATAN_PENGIRIM = $surat_masuk->getField("jabatan_pengirim");
		}
	}
	...
	function getAkses($suratMasukId, $userId, $infodivisi="", $infogantijabatan= "")
	{
		...
		if(!empty($infodivisi))
		{
			...
			$userId= str_replace("xx, xx", "'', ''", $userId);
			// echo $userId;exit;
			/*if(in_array("DIVISI", explode(",", $CI->USER_GROUP))) 
		    {
		    	$userId= $CI->NIP_BY_DIVISI;
	...
	/** Verify user login. True when login is valid**/
	function getAksesPermohonan($suratMasukId, $userId, $infodivisi="", $infogantijabatan= "")
	{
		// $CI = &get_instance();
	...
	function getkotakmasukbelum()
	{
		$CI = &get_instance();
		$CI->load->model("SuratMasuk");

		$surat_masuk = new SuratMasuk();
		$reqid= $CI->kauth->getInstance()->getIdentity()->ID;
		$reqNIPPP= "'".$reqid."'";
		$reqKelompokJabatann= "'".$CI->kauth->getInstance()->getIdentity()->KELOMPOK_JABATAN."'";
		$reqsatuankerjadivisi= $CI->kauth->getInstance()->getIdentity()->SATUAN_KERJA_ID_ASAL_ASLI;
		$requsergroup= $CI->kauth->getInstance()->getIdentity()->USER_GROUP;
		$reqsatuankerjaidasal= $CI->kauth->getInstance()->getIdentity()->SATUAN_KERJA_ID_ASAL;
		$reqcabangid= $CI->kauth->getInstance()->getIdentity()->CABANG_ID;
		$requserbantu= $CI->kauth->getInstance()->getIdentity()->USER_BANTU;

		$infogantijabatan= "";
		if(in_array("SURAT", explode(",", $requsergroup)))
		{
			$statement= " 
		...
		else
		{
			...
			(
				...
				(
					...
					EXISTS(SELECT 1 FROM SURAT_MASUK_ARSIP X WHERE X.SURAT_MASUK_ID = A.SURAT_MASUK_ID AND X.CABANG_ID = '".$reqcabangid."')
				)
			)";

			if($reqsatuankerjadivisi == $reqsatuankerjaidasal)
			{
				if(!empty($requserbantu))
				{
					$statement.= " AND B.SATUAN_KERJA_ID_TUJUAN = '".$reqsatuankerjaidasal."'";
				}
			}
			else
			{
				...
				AND
				(
					B.SATUAN_KERJA_ID_TUJUAN = '".$reqsatuankerjaidasal."'
					OR
					(
						B.SATUAN_KERJA_ID_TUJUAN = '".$reqcabangid."'
						and 
						exists
						(
							select 1
		...
		$statement.= " AND COALESCE(B.TERBACA_INFO, '')  NOT ILIKE '%".$reqid."%'";

		$jumlahkotakmasuksemua= 0;
		if($infogantijabatan == "1")
		{
			$jumlahkotakmasuksemua= $surat_masuk->getCountByParamsSuratMasuk(array(), $statement);	
		}
		else
		{
			$jumlahkotakmasuksemua= $surat_masuk->getCountByParamsNewSuratMasuk(array(), $reqNIPPP, $reqcabangid, $reqKelompokJabatann, $statement, $reqsatuankerjadivisi);
		}
		// echo $surat_masuk->query;exit;

		// echo $jumlahkotakmasuksemua;exit;
		$this->JUMLAH_KOTAK_MASUK_SEMUA= $jumlahkotakmasuksemua;
		// echo $jumlahkotakmasuksemua;
		// exit;
	}

	function getnewjumlahsurat($userId, $userGroup, $cabangId, $useridatasan, $userkelompok, $infogantijabatan, $cq="")
	{
		$CI = &get_instance();
		...
		/*$reqNIPPP= "'".$CI->ID."'";
		$reqKelompokJabatann= "'".$CI->KELOMPOK_JABATAN."'";
		$reqsatuankerjadivisi= $CI->SATUAN_KERJA_ID_ASAL_ASLI;
		...
		(
			...
			(
				A.STATUS_SURAT = 'TU-IN' AND
				EXISTS(SELECT 1 FROM SURAT_MASUK_ARSIP X WHERE X.SURAT_MASUK_ID = A.SURAT_MASUK_ID AND X.CABANG_ID = '".$CI->CABANG_ID."')
			)
		)";

		if($CI->SATUAN_KERJA_ID_ASAL_ASLI == $CI->SATUAN_KERJA_ID_ASAL)
		{
			if(!empty($CI->USER_BANTU))
			{
				$statement.= " AND B.SATUAN_KERJA_ID_TUJUAN = '".$CI->SATUAN_KERJA_ID_ASAL."'";
			}
		}
		else
		{
			...
			AND
			(
				B.SATUAN_KERJA_ID_TUJUAN = '".$CI->SATUAN_KERJA_ID_ASAL."'
				OR
				(
					B.SATUAN_KERJA_ID_TUJUAN = '".$CI->CABANG_ID."'
					and 
					exists
					(
						select 1
		...
		{
			$sOrder = "";
			$surat_masuk->jumlahNewSuratMasuk(array(), $reqNIPPP, $CI->CABANG_ID, $reqKelompokJabatann, $statement.$searchJson, $sOrder, $reqsatuankerjadivisi);
		}*/
