...
class CetakAgendaPDF
{
	var $reqId;
	var $reqRowId;
	var $reqMode;
	var $reqNamaDok;
	function generate($reqId, $reqRowId, $reqMode, $reqNamaDok)
	{
		$this->reqId = $reqId;
		$this->reqRowId = $reqRowId;
		$this->reqMode = $reqMode;
		$this->reqNamaDok = $reqNamaDok;
		...
		if ($this->reqNamaDok == "surat_masuk_manual") {
			// $urllink .= file_get_contents($CI->config->item('base_report') . "main/loadUrl/main/cetak_agenda_manual?reqMode=". $this->reqMode ."&reqId=". $this->reqId ."&reqRowId=". $this->reqRowId ."&reqNamaDok=". $this->reqNamaDok);
			$urllink=  $basereport."main/loadUrl/main/cetak_agenda_manual?reqMode=". $this->reqMode ."&reqId=". $this->reqId ."&reqRowId=". $this->reqRowId ."&reqNamaDok=". $this->reqNamaDok;
			
			// $urllink .= file_get_contents($CI->config->item('base_report') . "main/loadUrl/main/cetak_agenda_manual?reqMode=". $this->reqMode ."&reqId=". $this->reqId ."&reqRowId=". $this->reqRowId ."&reqNamaDok=". $this->reqNamaDok);
		} else {
			// $urllink .= file_get_contents($CI->config->item('base_report') . "main/loadUrl/main/cetak_agenda?reqMode=". $this->reqMode ."&reqId=". $this->reqId ."&reqRowId=". $this->reqRowId ."&reqNamaDok=". $this->reqNamaDok);
			$urllink=  $basereport."main/loadUrl/main/cetak_agenda?reqMode=". $this->reqMode ."&reqId=". $this->reqId ."&reqRowId=". $this->reqRowId ."&reqNamaDok=". $this->reqNamaDok;
			// $urllink .= file_get_contents($CI->config->item('base_report') . "main/loadUrl/main/cetak_agenda?reqMode=". $this->reqMode ."&reqId=". $this->reqId ."&reqRowId=". $this->reqRowId ."&reqNamaDok=". $this->reqNamaDok);
		}
