...
(
	(
		A.USER_ATASAN_ID = '".$this->ID."' AND  A.APPROVAL_DATE IS NULL
	) 
	OR 
	(
		A.USER_ATASAN_ID = '".$this->USER_GROUP.$this->ID."' AND A.APPROVAL_DATE IS NOT NULL 
	)
	OR 
	(
		A.USER_ID = '".$this->ID."' AND A.APPROVAL_DATE IS NULL AND A.STATUS_SURAT IN ('PEMBUAT')
	)
) 
...
$sOrder= "";
// $set->selectByParamsPersetujuan(array("A.SURAT_MASUK_ID"=>$reqId), -1, -1, $this->ID, $statement, $sOrder);
$set->selectByParamsNewPersetujuan(array("A.SURAT_MASUK_ID"=>$reqId), -1, -1, $this->ID, $this->USER_GROUP, $statement, $sOrder, $reqId, $satuankerjaganti);
$set->firstRow();
 // echo $set->query;exit;
$checkid= $set->getField("SURAT_MASUK_ID");
...
// $setlast->selectByParamsInfoLastNomorSurat(array("A.SURAT_MASUK_ID"=>$reqId), -1, -1);
// $setlast->firstRow();
...
if (empty($checkid))
{
	...
	$checkpernahsetujui= $set->getCountByParamsStatus(array("A.SURAT_MASUK_ID"=>$reqId), $this->ID, $statement);
	// echo $set->query;exit;
...
$arrlink= $suratmasukinfo->infolinkdetil($infojenisnaskahid);
$infolinkdetil= $arrlink["linkstatusdetil"];
$infolinkedit= $arrlink["linkstatusedit"];
...
<head>
	...
	<script type="text/javascript">
		...
		{
		?>
			$.get( "app/getJumlahSurat/?reqId=<?=$reqId?>&reqJenisSurat=<?=$reqJenisSurat?>", function( data ) {
		...
		function setdetil()
		{
			document.location.href = 'main/index/<?=$infolinkdetil?>/?reqId=<?=$reqId?>';
		}

		function setubah()
		{
			document.location.href = 'main/index/<?=$infolinkedit?>?reqMode=perlu_persetujuan_detil&reqId=<?=$reqId?>';
		}
		...
		function submitForm(reqStatusSurat) 
		{
	        ...
	        {
	        	urllink= "web/surat_masuk_json/revisi";
	        	reqMode= "manual";
	        ...
	        {
	        	urllink= "web/surat_masuk_json/logparaf";
	        	reqMode= "";
	        ...
	        {
	        	urllink= "web/surat_masuk_json/logposting";
	        	reqMode= "";
		...
		function setinfonomorsesuaitanggalentri()
		{
			...
			infotanggalajax= "web/surat_masuk_json/setinfonomorsesuaitanggalentri?v=<?=$reqId?>&t="+tanggalapproval;
			$.ajax({
				url: infotanggalajax,
				type: "GET",
				dataType: "json",
		...
		function setsimpan()
		{
			...
			$.ajax({
		        ...
		        success: function (response) {
		        	...
		        	if(response == "0")
		        	{
		        		// $.alert('<span style= color:red>gagal posting, karena nomor surat sudah ada</span>');
		        		$.alert({
			                title: 'Info',
...
<body style="overflow:hidden;">
	...
	<div id="wadah">
		<div class="judul-halaman">
            ...
            <!--<div class="dropdown yamm-fw notifikasi pull-right"><a href="#" data-toggle="dropdown" class="dropdown-toggle"><div class="test"></div></a>
                <ul class="dropdown-menu">
                    <li>
                        <div class="area-3dots-menu">
                            <div><a href="javascript:void(0)" onclick="setagenda()">Agenda Surat</a></div>
