...
$arrjenissurat= array(
  array("id"=>"suratmasukmanual", "nama"=>"Surat Masuk Manual")
  , array("id"=>"suratedaran", "nama"=>"Surat Edaran")
  , array("id"=>"suratkeluar", "nama"=>"Surat Keluar")
  , array("id"=>"notadinas", "nama"=>"Nota Dinas")
);
...
$arrdata= array(
	array("label"=>"JENIS SURAT", "width"=>"100")
	, array("label"=>"NO. SURAT", "width"=>"100")
	, array("label"=>"PEMBUAT", "width"=>"100")
	, array("label"=>"TUJUAN", "width"=>"80")
	...
	<script type="text/javascript" charset="utf-8">
		...
		$(document).ready(function() {
			...
			oTable = $('#example').dataTable({
				...
				"bProcessing": true,
				"bServerSide": true,
				"sAjaxSource": "web/histori_surat_json/jsonsuratmasuk?reqJenisNaskahId=<?=$reqJenisNaskahId?>&reqTahun=<?=$reqTahun?>",
			...
			$("#example tbody").click(function(event) {
				...
				if(anSelectedId !== "")
				window.location = ("<?= base_url() ?>main/index/histori_surat_detil/?reqMode=histori_surat&reqId="+anSelectedId);
			});

			$("#reqTahun,#reqJenisNaskahId,#reqPilihan").change(function() {
				$('#example_filter input').val("");
				setCariInfo();
			});

			$('#btnCari').on('click', function () {
				var reqTahun= reqPencarian= reqJenisNaskahId= "";
				reqTahun= $("#reqTahun").val();
				reqJenisNaskahId= $("#reqJenisNaskahId").val();
				reqPencarian= $('#example_filter input').val();
				reqPilihan= $("#reqPilihan").val();

				oTable.fnReloadAjax("web/histori_surat_json/jsonsuratmasuk?reqTahun="+reqTahun+"&reqPencarian="+reqPencarian+"&reqJenisNaskahId="+reqJenisNaskahId+"&reqPilihan="+reqPilihan);
			});
			...
			$('#btnCetak').on('click', function () {
				...
				newWindow = window.open("<?= base_url() ?>app/loadUrl/main/histori_surat_export/?reqJenisNaskahId=<?=$reqJenisNaskahId?>&reqTahun="+reqTahun+"&reqPencarian="+reqPencarian+"&reqPilihan="+reqPilihan, 'Cetak');
				newWindow.focus();
			});
	...
	<div id="wadah">
		...
		<div id="bluemenu" class="bluetabs">
        	<ul>
                ...
                <input type="hidden" id="reqStatusSurat" value="" />
                <li class="pull-right">
                	<select id="reqJenisNaskahId">
                		<option value="" >Semua</option>
                		<?
                		for($index= 0; $index < count($arrjenissurat); $index++)
                		{
                			$infoid= $arrjenissurat[$index]["id"];
                			$infotext= $arrjenissurat[$index]["nama"];
                		?>
                        <option value="<?=$infoid?>"><?=$infotext?></option>
                		<?
                		}
                		?>
                    </select>
