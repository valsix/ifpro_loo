...
else
{
    ...
    $reqDivisiId= $user_login->getField("DIVISI_ID");
}
?>
...
$(function(){
	$('#ff').form({
		url:'<?=base_url()?>web/user_login_json/add',
		onSubmit:function(){
			return $(this).form('validate');
		},
		success:function(data){
			$.messager.alertLink('Info', data, 'info','main/index/user_pengelola');	
...
<body class="bg-kanan-full">
            ...
            <form id="ff" method="post" novalidate enctype="multipart/form-data">
                <table class="table">
                    <thead>
                        ...
                        <tr>
                        	...
                        	<td>
                                ...
                                <input type="text" name="reqUserGroupPilih" class="easyui-combotree"  id="reqUserGroupPilih" 
                						...
                						textField:'text', editable:false,url:'combo_json/comboUserGroup',multiple:true,value:['<?=$reqUserGroupPilih?>'],
                                            onClick: function(node){
                                                var values= $('#reqUserGroupPilih').combotree('getValues');
                                                // $('#reqUserGroupId').val(rec.id);
                                                $('#reqUserGroupId').val(values);
                                            },
                        ...
                        <tr id="infogroupdivisi">
                            <td>Group Divisi</td>
                            <td>:</td>
                            <td>
                                <input type="hidden" name="reqResetDivisiId" />
                                <input type="text" id="reqDivisiId" class="easyui-combotree" name="reqDivisiId" data-options="
                                width:'500'
                                , panelHeight:'120'
                                , valueField:'id'
                                , textField:'text'
                                , url:'web/satuan_kerja_json/combotreesatker/'
                                , prompt:'Tentukan Pengirim...'," value="<?=$reqDivisiId?>"
                                />
                            </td>
                        </tr>
    ...
    <script type="text/javascript">
        function tambahPegawaiPengganti(id,text){
            $("#reqPegawaiId").val(id);
              $("#reqNama").val(text);

        }

        setsatuankerjadivisi();
        function setsatuankerjadivisi()
        {
            reqUserGroupId= $('#reqUserGroupId').val();
            $("#reqResetDivisiId").val();
            $("#infogroupdivisi").hide();

            let str = reqUserGroupId;
            if(str.indexOf("DIVISI") > 0)
            {
                $("#reqResetDivisiId").val("1");
                $("#infogroupdivisi").show();
            }
            else
            {
                $('#reqDivisiId').combotree('setValue', '');
            }
        }
    </script>
</body>
