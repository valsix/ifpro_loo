...
{
    $set= new Disposisi();
    $set->selectByParams(array("A.DISPOSISI_ID"=>$reqRowId));
    $set->firstRow();
    // echo $set->query;exit;
    // $infoparentid= $set->getField("DISPOSISI_PARENT_ID");
    $namasatuankerja= $set->getField("NAMA_SATKER");
    // echo $namasatuankerja;exit;

    $infosql= " select ambilposisidisposisi(".$reqRowId.", 0) infodataid";
    $hasilsql = $this->db->query($infosql)->row();
    $infoparentid= $hasilsql->infodataid;

    $infosql= " select ambildataposisidisposisi(".$reqRowId.", 0, '') infodatadisposisiid";
    $hasilsql = $this->db->query($infosql)->row();
    $infodatadisposisiid= $hasilsql->infodatadisposisiid;
}
// echo $infoparentid;exit;

$statementdetil= "";
if(!empty($infodatadisposisiid))
{
    $statementdetil= " AND A.DISPOSISI_ID IN (".$infodatadisposisiid.")";
}

$arrdisposisi= array();
$index_data= 0;
$set= new Disposisi();
$set->selectByParams(array("A.SURAT_MASUK_ID"=>$reqId),-1,-1, $statementdetil, "ORDER BY A.TANGGAL_DISPOSISI");
...
$infodisposisiid= "";
$arrayKey= in_array_column($infoparentid, "DISPOSISI_PARENT_ID", $arrdisposisi);
if(!empty($arrayKey))
{
