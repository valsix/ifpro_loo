...
$aColumns = array("PEGAWAI_ID", "NIP", "NAMA", "JABATAN", "UNIT KERJA", "DIREKTORAT", "JENIS","EMAIL");
?>
...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 25,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "web/pegawai_json/json/?reqCabangId=<?=$this->CABANG_ID?>",   
		...
		$(document).ready(function() {
			$('#reqDepartemen').combobox({
				onSelect: function(param) {
					oTable.fnReloadAjax("web/pegawai_json/json/?reqCabangId="+param.id+"&reqJenis="+$('#reqJenis').combobox("getValue"));
				}
			});

			$('#reqJenis').combobox({
				onSelect: function(param) {
					oTable.fnReloadAjax("web/pegawai_json/json/?reqCabangId="+$('#reqDepartemen').combobox("getValue")+"&reqJenis="+param.id);
				}
			});
        ...
        <ul>
			<li>
				<div style="padding-left: 10px" class="pull-left">
	                ...
	                <input name="reqDepartemen" class="easyui-combotree textbox form-control" id="reqDepartemen" value="<?=$this->CABANG_ID?>"
	                data-options="width:'400',panelHeight:'400',panelWidth:'500',editable:false,valueField:'id',textField:'text',url:'web/satuan_kerja_json/combo_cabang'"  />
	                <label>Jenis :</label>
	                <input class="easyui-combotree textbox form-control" id="reqJenis" value="<?=$reqJenis?>"
	                data-options="width:'100',panelHeight:'400',panelWidth:'100',editable:false,valueField:'id',textField:'text',url:'web/pegawai_json/combojenis'"  />
	                <!--<span style="right:0; margin-right:20px; position:absolute"><a id="btnAdd"> Sinkronisasi</a></span>-->
				</div>
            </li>
            <li class="pull-right">
