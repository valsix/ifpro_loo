...
if (!empty($reqId))
{
    ...
    elseif($reqStatusSurat == "PARAF" || $reqStatusSurat == "VALIDASI")
    {
        ...
        $infonextpemaraf= $set->getNextParaf(" AND A.SURAT_MASUK_ID = ".$reqIdDraft);
        // echo $infonextpemaraf;exit;
        // echo $set->query;exit;

        $set= new SuratMasuk();
        $statement= " AND ((A.USER_ATASAN_ID = '".$this->ID."' AND  A.APPROVAL_DATE IS NULL) OR (A.USER_ATASAN_ID = '".$this->USER_GROUP.$this->ID."' AND A.APPROVAL_DATE IS NOT NULL )) AND A.STATUS_SURAT IN ('PARAF', 'VALIDASI') AND A.SURAT_MASUK_ID = ".$reqIdDraft." AND COALESCE(A.NEXT_URUT,1) = ".$infonextpemaraf;
        $sOrder= "";
    ...
    <div class="judul-halaman bg-course">
    	...
    	<div class="btn-atas clearfix">
            ...
            ?>
                ...
                <button class="btn btn-primary btn-sm pull-right" type="button" onClick="submitForm('PARAF')"><i class="fa fa-check-square-o"></i> Setujui</button>
            <?
            }
    ...
    function setsimpan()
    {
        $('#ff').form('submit', {
            ...
            success: function(data) {
                ...
                {
                    $.messager.alertLink('Info', data, 'info', "app/loadUrl/main/nota_dinas_add?reqMode=<?=$reqLinkMode?>&reqId=<?=$reqId?>");
                }
