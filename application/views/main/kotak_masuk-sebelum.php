...
$set= new SuratMasuk();
$countterbaca= new SuratMasuk();
$countbelum= new SuratMasuk();
...
// $countterbaca->selectByParamsTerbaca( $this->ID, $this->USER_GROUP, $this->CABANG_ID, $this->ID_ATASAN, $this->KELOMPOK_JABATAN);
// $countterbaca->firstRow();
// $jumlahterbaca = $countterbaca->getField("JUMLAH_KOTAK_MASUK_SEMUA_TERBACA");

// $countbelum->selectByParamsBelumTerbaca( $this->ID, $this->USER_GROUP, $this->CABANG_ID, $this->ID_ATASAN, $this->KELOMPOK_JABATAN);
// $countbelum->firstRow();
// $jumlahbelum = $countbelum->getField("JUMLAH_KOTAK_MASUK_SEMUA");
...
// $jumlahbelum = $suratmasukinfo->JUMLAH_KOTAK_MASUK_SEMUA;
// $jumlahterbaca = $suratmasukinfo->JUMLAH_KOTAK_MASUK_SEMUA_TERBACA;

// echo $count->query;exit;
//echo $suratmasukinfo->JUMLAH_KOTAK_MASUK_SEMUA;;exit;
  // echo $jumlahterbaca;exit;
...
<head>
	...
	<script type="text/javascript" charset="utf-8">
		...
		$(document).ready(function() {
			...
			oTable = $('#example').dataTable({
				...
				"bProcessing": true,
				"bServerSide": true,
				"sAjaxSource": "web/surat_masuk_json/jsonsuratmasuk?reqJenisNaskahId=<?=$reqJenisNaskahId?>&reqTahun=<?=$reqTahun?>",
	        	...
	        	"fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
					...
					if( aData[icolor] == 'Rahasia')
					{
						color= "fdd6d6";
					}
					else if( aData[icolor] == 'Sangat Segera')
					{
						color= "ffeeba";
					}
					else if( aData[icolor] == 'Segera')
					{
						color= "b4ebff";
					}
			...
			$("#example tbody").click(function(event) {
				...
				if(anSelectedId !== "")
				window.location = ("<?= base_url() ?>main/index/kotak_masuk_detil/?reqMode=kotak_masuk&reqId="+anSelectedId+"&reqRowId="+anSelectedRowId+"&reqPilihan="+reqPilihan);
			});
			...
			$('#btnCari').on('click', function () {
				...
				oTable.fnReloadAjax("web/surat_masuk_json/jsonsuratmasuk?reqJenisNaskahId=<?=$reqJenisNaskahId?>&reqTahun="+reqTahun+"&reqPencarian="+reqPencarian+"&reqStatusSurat="+reqStatusSurat+"&reqPilihan="+reqPilihan);
			});
			...
			$('#btnCetak').on('click', function () {
				...
				newWindow = window.open("<?= base_url() ?>app/loadUrl/main/kotak_masuk_export/?reqJenisNaskahId=<?=$reqJenisNaskahId?>&reqTahun="+reqTahun+"&reqPencarian="+reqPencarian+"&reqPilihan="+reqPilihan, 'Cetak');
				newWindow.focus();
			});
  ...
  //       $(function(){
		// 	$("#reqPilihan").on('change', function(){
		// 		reqPilihan= $("#reqPilihan").val();
		// 		reqTahun= $("#reqTahun").val();
		// 	   	oTable.fnReloadAjax("web/surat_masuk_json/jsonsuratmasuk?reqJenisNaskahId=<?=$reqJenisNaskahId?>&reqTahun="+reqTahun+"&reqPilihan="+reqPilihan);
		// 	})
		// });
	</script>
        	...
        	<ul>
                ...
                <li class="pull-right">
                	<select id="reqStatusSurat">
                       ...
                       <!--  <option value="1" >Sudah dibaca(<?=$jumlahterbaca?>)</option>
                        <option value="2" >Belum dibaca (<?=$jumlahbelum?>)</option> -->
