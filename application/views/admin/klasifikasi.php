    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
		  ...
		  $('#btnAdd').on('click', function () {
			  window.parent.openPopup("<?=base_url()?>app/loadUrl/admin/<?=$reqFilename?>_add");
		  ...
		  $('#btnEdit').on('click', function () {
			  if(anSelectedData == "")
				  return false;				
			  window.parent.openPopup("<?=base_url()?>app/loadUrl/admin/<?=$reqFilename?>_add/?reqId="+anSelectedId);
			...
			$('#btnAktifkan').on('click', function () {
				...
				konfirmasiAksi("Aktifkan klasifikasi beserta klasifikasi di bawahnya?", "web/klasifikasi_json/aktif", anSelectedId);
			...
			$('#btnNonAktifkan').on('click', function () {
				...
				konfirmasiAksi("Non-aktifkan klasifikasi beserta klasifikasi di bawahnya?", "web/klasifikasi_json/nonaktif", anSelectedId);
			...
			$('#btnDelete').on('click', function () {
				...
				deleteData("web/klasifikasi_json/delete", anSelectedId);
	    ...
	    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
	        <ul>
				<li>
	                ...
	                <a id="btnAktifkan" title="Hapus"><img src="<?=base_url()?>images/icon-hapus.png" /> Aktifkan</a>        
	                <a id="btnNonAktifkan" title="Hapus"><img src="<?=base_url()?>images/icon-hapus.png" /> Non-Aktifkan</a>       
	                <span style="margin-left:150px;">
	                <label>Status :</label>
	                <input name="reqStatus" class="easyui-combobox textbox form-control" id="reqStatus" value="1"
	        			data-options="width:'200',panelHeight:'100',editable:false,valueField:'id',textField:'text',url:'combo_json/comboStatusKlasifikasi'"  />
	                </span> 
	            </li>        
	        </ul>
	    </div>
	    <table id="treeSatker" class="easyui-treegrid" style="width:1270px;height:600px"
	            data-options="
	                url: 'web/klasifikasi_json/treetable/',
	                pagination: true,            
	                pageSize: 50,
					pageList: [50, 100],
	        ...
	        <thead>
	            <tr>                                
	                <th data-options="field:'text'" width="1000" >Klasifikasi</th>
	                <th data-options="field:'STATUS_AKTIF_DESC'" width="270" >Status</th>
	            </tr>
	        </thead>
	    </table>
	</div>
