...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 100,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>rekapitulasi_json/json_uang_makan/?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>&reqDepartemen=<?=$reqDepartemen?>",			  
			...
			/* Click event handler */
			  ...
			  /*$("#reqBulan, #reqTahun").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_json/json_uang_makan/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+$('#reqDepartemen').combotree('getValue'));
			  });*/
			  
			  $("#reqBulan").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_json/json_uang_makan/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'));
			  });
			 ...
			 $("#reqTahun").change(function() {
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_json/json_uang_makan/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+param.id);				 	 
					}
			  });
			  
			  $('#btnCetak').on('click', function () {
				  newWindow = window.open('rekapitulasi_uang_makan_cetak.php?reqBulan='+$("#reqBulan").val()+'&reqTahun='+ $("#reqTahun").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'), 'Cetak');
				  newWindow.focus();
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Rekapitulasi Uang Makan</div>
    ...
    <thead>
            <tr>
                <th width="170px">NRP</th> 
                <th width="350px">Nama</th> 
                <th width="350px">Jabatan</th> 
                <th width="350px">Jatah Uang Makan</th>
            </tr>
    </thead>
    </table> 
</div>
