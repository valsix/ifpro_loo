...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>import_cuti_tahunan_json/json?reqDepartemen=<?=$reqDepartemen?>",		  
			...
			/* Click event handler */
			  ...
			  $('#btnAdd').on('click', function () {
			   ...
			   $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("<?=base_url()?>import_cuti_tahunan_json/delete/"+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("<?=base_url()?>import_cuti_tahunan_json/json?reqDepartemen=<?=$reqDepartemen?>");
							});
			  ...
			  $('#btnImport').on('click', function () {
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/import_cuti_tahunan_import/");
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>import_cuti_tahunan_json/json?reqDepartemen="+param.id);	
					}
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Import Cuti Tahunan</div>
