...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>rekapitulasi_kehadiran_pegawai_tahun_json/json/?reqTahun=<?=$reqTahun?>&reqDepartemen=<?=$reqDepartemen?>",			  
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/rekapitulasi_kehadiran_pegawai_tahun_cetak_add/"+anSelectedId);
			  ...
			  $("#reqBulan, #reqTahun").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_kehadiran_pegawai_tahun_json/json/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'));				 	 
			  });
			  
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_kehadiran_pegawai_tahun_json/json/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+param.id);
					}
			  });
			  ...
			  $('#btnCetak').on('click', function () {
				  newWindow = window.open('rekapitulasi_kehadiran_pegawai_tahun_cetak.php?reqTahun='+ $("#reqTahun").val()+'&reqDepartemen='+$('#reqDepartemen').combotree('getValue'), 'Cetak');
				  newWindow.focus();
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Monitoring Kehadiran Tahunan</div>
    ...
    <thead>
            ...
            <th width="80px">Efektif</th>
            <th width="80px">Alpha</th>
            <th width="80px">Ijin</th>
            <th width="80px">Cuti Tahunan</th>
            <th width="80px">Cuti Lainnya</th>
        </tr>
    </thead>
    </table> 
</div>
