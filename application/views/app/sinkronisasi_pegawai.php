...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>sinkronisasi_pegawai_json/json/?reqDepartemen=<?=$reqDepartemen?>",		  
			...
			/* Click event handler */
			  ...
			  /*$('#btnAdd').on('click', function () {
				  document.location.href = "app/index/sinkronisasi_pegawai_add";
			  ...
			  $('#btnSinkronisasi').on('click', function () {
					...
					if(confirm("Apakah anda yakin untuk memperbarui data pegawai ?"))
					{
						var win = $.messager.progress({
							title:'Proses generate data pegawai.',
							msg:'Proses data...'
						});									
						var jqxhr = $.get("<?=base_url()?>sinkronisasi_pegawai_json/proses/", function(data) {
							
							oTable.fnReloadAjax("<?=base_url()?>sinkronisasi_pegawai_json/json/?reqDepartemen=<?=$reqDepartemen?>");
			  ...
			  $('#btnSinkronisasiDl').on('click', function () {
					
					if(confirm("Apakah anda yakin untuk memperbarui data DL ?"))
					{
						var win = $.messager.progress({
							title:'Proses generate data DL.',
							msg:'Proses data...'
						});									
						var jqxhr = $.get("<?=base_url()?>sinkronisasi_pegawai_json/prosesDl/", function(data) {
			  ...
			  $('#btnAdd').on('click', function () {
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/pegawai_add/");
			  ...
			  $('#btnEditData').on('click', function () {
				  ...
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/pegawai_add/?reqId="+anSelectedId);
			  ...
			  $('#btnEdit').on('click', function () {
				  ...
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/sinkronisasi_pegawai_add/?reqId="+anSelectedId+"&reqMode=view");
			   ...
			   $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("permohonan_cuti_tahunan_json/delete/"+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("permohonan_cuti_tahunan_json/json/?reqStatusAgenda=" +$("#reqStatusAgenda").val());
							});
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>sinkronisasi_pegawai_json/json/?reqDepartemen="+param.id);	
					}
			  });
...
<div id="wadah">
    <div class="judul-halaman">Sinkronisasi Pegawai</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
			<li>
            	<!--<span><a href="#" id="btnAdd" title="Tambah"><img src="<?=base_url()?>images/icon-tambah.png" /> Tambah</a></span>
                <span><a href="#" id="btnEditData" title="Ubah Data"><img src="<?=base_url()?>images/icon-edit.png" /> Ubah Data</a> </span>-->
                <span><a id="btnEdit" title="Ubah Finger"><img src="<?=base_url()?>images/icon-edit.png" /> Ubah Finger</a> </span>
            </li>    
            <li>
            	<a id="btnSinkronisasi" title="Sinkronisasi"><img src="<?=base_url()?>images/icon-sinkronisasi.png" /> Sinkronisasi</a>
            </li>     
            <li>
            	<a id="btnSinkronisasiDl" title="Sinkronisasi DL"><img src="<?=base_url()?>images/icon-sinkronisasi.png" /> Sinkronisasi DL</a>
            </li>   
