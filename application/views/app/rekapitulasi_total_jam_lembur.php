...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>rekapitulasi_json/json_total_jam_lembur/?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>&reqDepartemen=<?=$reqDepartemen?>",			  
			...
			/* Click event handler */
			  ...
			  $("#reqBulan").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_json/json_total_jam_lembur/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'));
			  });
			 ...
			 $("#reqTahun").change(function() {
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_json/json_total_jam_lembur/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+param.id);				 	 
					}
			  });
			  ...
			  $('#btnCetak').on('click', function () {
				  newWindow = window.open('rekapitulasi_lembur_cetak.php?reqBulan='+$("#reqBulan").val()+'&reqTahun='+ $("#reqTahun").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'), 'Cetak');
				  newWindow.focus();
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Rekapitulasi Jam Lembur</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
			<li>
                <a href="#" id="btnCetak" title="Cetak"> Cetak Rincian Lembur</a>     
            </li>        
        </ul>
    </div>
    ...
    <thead>
            <tr>
                ...
                <th width="100px">Total Jam</th>
                <!-- <th width="100px">Sisa Jam</th> -->
                <th width="100px">Jam Kerja</th>
                <th width="100px">Jam Libur</th>
            </tr>
    </thead>
    </table> 
</div>
