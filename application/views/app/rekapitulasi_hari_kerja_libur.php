...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>rekapitulasi_hari_kerja_libur_json/json/?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>",			  
			...
			/* Click event handler */
			  ...
			  $("#reqBulan, #reqTahun").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_hari_kerja_libur_json/json/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val());				 	 
			  });
...
<div id="wadah">
    <div class="judul-halaman">Rekapitulasi Bekerja pada Hari Kerja / Libur</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
			<li>
                <?php /*?><a href="#" id="btnCetak" title="Cetak"> Cetak</a><?php */?>     
            </li>        
        </ul>
    </div>
