...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>pegawai_lembur_json/json/?reqCabang=<?=$reqCabang?>&reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>",			      
			...
			/* Click event handler */
			  ...
			  $("#reqCabang").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>pegawai_lembur_json/json/?reqCabang="+$("#reqCabang").combobox("getValue")+"&reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqStatus="+$("#reqStatus").val());				 	 
			  });
			  
			  $("#reqBulan").change(function() {
				 fnReloadAjax("<?=base_url()?>pegawai_lembur_json/json/?reqCabang="+$("#reqCabang").combobox("getValue")+"&reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqStatus="+$("#reqStatus").val());				 	 
			  });
...
<div id="wadah">
    <div class="judul-halaman">Jadwal Pegawai Lembur</div>
