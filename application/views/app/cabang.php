...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>cabang_json/json",			  
			...
			/* Click event handler */
			  ...
			  $("#example tbody").click(function(event) {
			   ...
			   $('#btnSinkronisasiCabang').on('click', function () {
					
					if(confirm("Apakah anda yakin untuk memperbarui data cabang ?"))
					{
						var win = $.messager.progress({
							title:'Proses generate data cabang.',
							msg:'Proses data...'
						});									
						var jqxhr = $.get("<?=base_url()?>sinkronisasi_pegawai_json/prosesCabang/", function(data) {
							
							 oTable.fnReloadAjax("<?=base_url()?>cabang_json/json");
			  ...
			  $('#btnAdd').on('click', function () {
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/cabang_add");
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/cabang_add/?reqId="+anSelectedId);
			  ...
			  $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("<?=base_url()?>cabang_json/delete/"+anSelectedId,
							  function(data){
			  ...
			  $("#reqStatus").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>cabang_json/json/?reqStatus=" +$("#reqStatus").val());				 	 
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Data Unit</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
			<li>
             ...
             <li>
            	<a href="#" id="btnSinkronisasiCabang" title="Sinkronisasi Cabang"><img src="<?=base_url()?>images/icon-sinkronisasi.png" /> Sinkronisasi Cabang</a>
            </li>        
           <!-- <li>
            	<a href="#" id="" title="Tambah"><img src="<?=base_url()?>images/icon-sinkronisasi.png" /> Sinkronisasi</a>
            </li>-->
        </ul>
    </div>
    ...
    <thead>
        <tr>
            ...
            <th width="70px">No Telepon</th>
            <th width="70px">No Fax</th>
            <th width="70px">Latitude</th>
            <th width="70px">Longitude</th>
            <th width="70px">Radius</th>
            <th width="70px">Status Proyek</th>
        </tr>
     </thead>
    </table> 
</div>
