...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "jam_kerja_cabang_json/json",			  
			...
			/* Click event handler */
			  ...
			  $('#btnAdd').on('click', function () {
				  //top.openPopup("app/loadUrl/app/jam_kerja_add");
				  window.parent.parent.openPopup("loadUrl/app/jam_kerja_cabang_add");
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  top.openPopup("loadUrl/app/jam_kerja_cabang_add/?reqId="+anSelectedId);
			  ...
			  $('#reqCabangId').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("jam_kerja_cabang_json/json/?reqCabangId="+param.id);	
					}
			  });
			  ...
			  $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("jam_kerja_cabang_json/delete/"+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("jam_kerja_cabang_json/json/?reqCabangId="+$('#reqCabangId').combotree('getValue'));
							});
...
<div id="wadah">
    <div class="judul-halaman"><span>Data Master Jam Kerja Regular</span></div>
