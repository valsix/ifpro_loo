...
$fname = tempnam("/tmp", "cetak_rekapitulasi_sisa_istirahat.xls");
$workbook = & new writeexcel_workbookbig($fname);
$worksheet = &$workbook->addworksheet();

$rekapitulasi_sisa_istirahat = new PermohonanIstirahat();
$cabang = new Cabang();
...
$statement .= "AND NOT EXISTS(SELECT 1 FROM PEGAWAI_JATAH_CUTI_TAHUNAN X WHERE X.PEGAWAI_ID = A.PEGAWAI_ID AND TANGGAL_BERLAKU <= TRUNC(SYSDATE))";

$rekapitulasi_sisa_istirahat->selectByParamsJatahIstirahatReport(array(), -1, -1, $statement," ORDER BY B.NAMA ASC");
$cabang->selectByParams(array("CABANG_ID" => $reqCabangId), -1, -1);
$cabang->firstRow();
//echo $rekapitulasi_sisa_istirahat->query; exit;
...
$worksheet->write(1, 4, "REKAP SISA ISTIRAHAT ".$reqTahun, $text_format);
...
$row = 5;
while($rekapitulasi_sisa_istirahat->nextRow())
{
	$worksheet->write($row, 1, $rekapitulasi_sisa_istirahat->getField('PEGAWAI_ID'), $text_format_line_left);
	$worksheet->write($row, 2, $rekapitulasi_sisa_istirahat->getField('NAMA'), $text_format_line_left);
	$worksheet->write($row, 3, $rekapitulasi_sisa_istirahat->getField('STATUS_PEGAWAI'), $text_format_line_left);
	$worksheet->write($row, 4, $rekapitulasi_sisa_istirahat->getField('TANGGAL_MASUK'), $text_format_line);
	$worksheet->write($row, 5, $rekapitulasi_sisa_istirahat->getField('TOTAL_CUTI'), $text_format_line);
	$worksheet->write($row, 6, $rekapitulasi_sisa_istirahat->getField('TOTAL_DIAMBIL'), $text_format_line);
	$worksheet->write($row, 7, $rekapitulasi_sisa_istirahat->getField('TOTAL_VERIFIKASI'), $text_format_line);
	$worksheet->write($row, 8, $rekapitulasi_sisa_istirahat->getField('TOTAL_SETUJU'), $text_format_line);
	$worksheet->write($row, 9, $rekapitulasi_sisa_istirahat->getField('TOTAL_TOLAK'), $text_format_line);
	$worksheet->write($row, 10, $rekapitulasi_sisa_istirahat->getField('TOTAL_SISA'), $text_format_line);
	$row++;
}

$workbook->close();

header("Content-Type: application/x-msexcel; name=\"cetak_rekapitulasi_sisa_istirahat.xls\"");
header("Content-Disposition: inline; filename=\"cetak_rekapitulasi_sisa_istirahat.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>