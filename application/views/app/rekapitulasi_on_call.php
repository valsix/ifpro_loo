...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>rekapitulasi_oncall_json/json/?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>&reqDepartemen=<?=$reqDepartemen?>",			  
			...
			/* Click event handler */
			  ...
			  $("#reqBulan, #reqTahun").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_oncall_json/json/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'));				 	 
			  });
			  
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_oncall_json/json/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+param.id);
					}
			  });
			  ...
			  $('#btnCetak').on('click', function () {
				  newWindow = window.open('rekapitulasi_on_call_cetak.php?reqBulan='+$("#reqBulan").val()+'&reqTahun='+ $("#reqTahun").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'), 'Cetak');
				  newWindow.focus();
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Rekapitulasi On Call</div>
    ...
    <thead>
            <tr>
                <th width="70px" rowspan="3"  style="text-align:center">NRP</th> 
                <th width="450px" rowspan="3"  style="text-align:center">Nama</th> 
                <th colspan="31" style="text-align:center">Tanggal</th>  
                <th width="70px" rowspan="3"  style="text-align:center">Jumlah Piket</th> 
                <th width="70px" colspan="2"  style="text-align:center">KETERANGAN</th>
            </tr>
            <tr>
                <?
                 $x=1;
                 while ($x <= $date) {
                 ?>
                 <th width="10px" style="text-align:center" rowspan="2"><?=$x?></th>
                 <?	
                    $x++;
                 }
                 ?> 
                <th width="70px" colspan="2"  style="text-align:center">KENDARAAN</th>
            </tr>
            <tr>
                <th width="70px"  style="text-align:center">DINAS</th>  
                <th width="70px"  style="text-align:center">PRIBADI</th>              
            </tr>
    </thead>
    </table> 
</div>
