...
if(varItem == 'permohonan_jadwal_keandalan_add_data'){
	$("#permohonan_jadwal_keandalan_add_data").addClass("aktif");
	$('#permohonan_jadwal_keandalan_add_data').css({'background-position': '0 -27px'});
	mainFramePop.location.href='app/loadUrl/app/permohonan_jadwal_keandalan_add_data/?reqMode=<?=$reqMode?>&reqId=<?=$reqId?>';
	document.getElementById('trdetil').style.display = 'none';	
}
else if(varItem == 'permohonan_jadwal_keandalan_add_jadwal'){
	$("#permohonan_jadwal_keandalan_add_jadwal").addClass("aktif");
	$('#permohonan_jadwal_keandalan_add_jadwal').css({'background-position': '0 -27px'});
	mainFramePop.location.href='app/loadUrl/app/permohonan_jadwal_keandalan_add_jadwal/?reqId=<?=$reqId?>';
	document.getElementById('trdetil').style.display = 'none';	
}
else if(varItem == 'permohonan_jadwal_keandalan_add_pegawai'){
	$("#permohonan_jadwal_keandalan_add_pegawai").addClass("aktif");
	$('#permohonan_jadwal_keandalan_add_pegawai').css({'background-position': '0 -27px'});
	mainFramePop.location.href='app/loadUrl/app/permohonan_jadwal_keandalan_add_pegawai/?reqId=<?=$reqId?>';
	document.getElementById('trdetil').style.display = 'none';	
}
return true;
}
	...
	<div id="wrapper" class="bg-kiri-popup" style="overflow:hidden;">
        ...
        <nav class="navbar navbar-default navbar-static-top popup-atas" role="navigation" style="margin-bottom: 0;">
            <div class="navbar-header navbar-header-popup">
                ...
                <a class="navbar-brand">Permohonan Keandalan</a>
            </div>
            <!-- /.navbar-header -->
                ...
                <div class="sidebar-nav navbar-collapse">
                    <ul class="nav" id="side-menu">
                        <li style="border:none;">
                            ...
                            <div id="menu-kiri">
                                <div><a id="permohonan_jadwal_keandalan_add_data" onClick="executeOnClick('permohonan_jadwal_keandalan_add_data');" class="aktif">Data Permohonan</a></div>                                
                                <div><a id="permohonan_jadwal_keandalan_add_jadwal" onClick="executeOnClick('permohonan_jadwal_keandalan_add_jadwal');">Jadwal Keandalan</a></div>
                                <div><a id="permohonan_jadwal_keandalan_add_pegawai" onClick="executeOnClick('permohonan_jadwal_keandalan_add_pegawai');">Daftar Pegawai</a></div>
                            </div>
                        </li>
                    </ul>
                </div>
        ...
        <div class="konten-utama-popup">
        	<?php /*?><div class="frame-atas">
            	haiii atas
            </div>
            <div class="frame-bawah">
            	haiii bawah
            </div><?php */?>
			<table cellpadding="0" cellspacing="0" style="width:100%; height:100%;">
                <tr height="50%">
                    <td><iframe src="app/loadUrl/app/permohonan_jadwal_keandalan_add_data/?reqMode=<?=$reqMode?>&reqId=<?=$reqId?>" class="mainframe" id="idMainFrame" name="mainFramePop" width="100%" height="100%" scrolling="auto" frameborder="0" style="display:block;"></iframe></td>
                </tr>
