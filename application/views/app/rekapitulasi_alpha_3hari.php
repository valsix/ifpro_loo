...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>rekapitulasi_kehadiran_json/json_alpha/?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>&reqDepartemen=<?=$reqDepartemen?>",			  
			...
			/* Click event handler */
			  ...
			  $("#reqBulan, #reqTahun").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_kehadiran_json/json_alpha/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'));				 	 
			  });
			  
			  
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_kehadiran_json/json_alpha/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+param.id);				 	 
					}
			  });
...
<div id="wadah">
    <div class="judul-halaman">Monitoring Alpha Lebih 3 Hari</div>
    ...
    <thead>
            <tr>
                ...
                <th width="100px">Jumlah Alpha</th>
                <th>Alpha Hari</th>
            </tr>
    </thead>
    </table> 
