...
$fname = tempnam("/tmp", "rekapitulasi_kehadiran_proyek_pegawai_ttp_cetak.xls");
...
$reqPermohonanProyekId = $this->input->get("reqId");

$statement = " AND A.PERMOHONAN_PROYEK_ID = '".$reqPermohonanProyekId."' AND F.PERIODE = '".$reqPeriode."' ";

$permohonan_proyek->selectByParamsKehadiranProyek(array(), -1, -1, $statement, " ORDER BY D.NAMA ASC");
//echo $permohonan_proyek->query; exit;
...
$reqAwal = $permohonan_proyek_nama->getField("AWAL");
$reqAkhir = $permohonan_proyek_nama->getField("AKHIR");
$reqPeriodeAwal = $permohonan_proyek_nama->getField("PERIODE_AWAL");
$reqPeriodeAkhir = $permohonan_proyek_nama->getField("PERIODE_AKHIR");

if($reqPeriodeAwal == $reqPeriodeAkhir)
{
	$reqAwal = $reqAwal;
	$reqAkhir = $reqAkhir;
}
...
{
	$reqAwal = 1;
	$reqAkhir = cal_days_in_month(CAL_GREGORIAN, date(getBulanPeriode($reqPeriode)), date(getTahunPeriode($reqPeriode)));
}

$permohonan_proyek_cek->selectByParams(array("A.PERMOHONAN_PROYEK_ID" => $reqPermohonanProyekId), -1, -1);

$jumHari = cal_days_in_month(CAL_GREGORIAN, date(getBulanPeriode($reqPeriode)), date(getTahunPeriode($reqPeriode)));
...
header("Content-Type: application/x-msexcel; name=\"rekapitulasi_kehadiran_proyek_pegawai_ttp_cetak.xls\"");
header("Content-Disposition: inline; filename=\"rekapitulasi_kehadiran_proyek_pegawai_ttp_cetak.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>