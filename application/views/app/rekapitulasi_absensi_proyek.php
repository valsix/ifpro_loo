    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			 ...
			 "aoColumns": [
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>absensi_rekap_proyek_json/rekapitulasi_absensi_proyek_json/?reqPeriode=<?=$reqPeriode?>&reqPermohonanProyekId=<?=$reqPermohonanProyekId?>&reqAwal=<?=$reqAwal?>&reqAkhir=<?=$reqAkhir?>",		
			...
			/* Click event handler */
			  ...
			  $('#btnCetakPeriode').on('click', function () {
				  ...
				  if($("#reqDepartemen").combotree('getValue') == "")
				  {
					  $.messager.alert('Info','Pilih periode proyek.');
					  return false;
				  }
				  
				  newWindow = window.open('<?=base_url()?>app/loadUrl/app/rekapitulasi_presensi_proyek_periode_cetak.php?reqPeriode=<?=$reqPeriode?>&reqPermohonanProyekId=<?=$reqPermohonanProyekId?>&reqAwal=<?=$reqAwal?>&reqAkhir=<?=$reqAkhir?>', 'Cetak');
				  newWindow.focus();
			  });
			  ...
			  $('#btnCetakProyek').on('click', function () {
				  ...
				  newWindow = window.open('<?=base_url()?>app/loadUrl/app/rekapitulasi_presensi_proyek_cetak.php?reqPermohonanProyekId=<?=$reqPermohonanProyekId?>&reqAwal=<?=$reqAwal?>&reqAkhir=<?=$reqAkhir?>', 'Cetak');
				  newWindow.focus();
			  });
...
<div id="wadah">
    <div class="judul-halaman">Rekapitulasi Presensi Jam Kerja Proyek : <?=$reqProyek?></div>
    ...
    <div id="parameter-tambahan">
        Proyek Periode : <input id="reqDepartemen" class="easyui-combotree" name="reqDepartemen" data-options="url:'<?=base_url()?>permohonan_proyek_json/combotree_jadwal',
					valueField:'id',
