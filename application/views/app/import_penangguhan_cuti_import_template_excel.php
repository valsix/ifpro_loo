...
$fname = tempnam("/tmp", "import_penangguhan_cuti.xls");
...
for ($i=0; $i < 2; $i++)
{
	...
	if ($i==0)
	{
		...
		$worksheet->set_column(4, 4, 14.00);
			...
			$worksheet->write(0, 2, "TAHUN", $text_format_line_bold);
			$worksheet->write(0, 3, "LAMA PENANGGUHAN", $text_format_line_bold);
...
header("Content-Type: application/x-msexcel; name=\"import_penangguhan_cuti_import_template.xls\"");
header("Content-Disposition: inline; filename=\"import_penangguhan_cuti_import_template.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>