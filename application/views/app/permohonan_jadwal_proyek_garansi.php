...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>permohonan_proyek_json/json_garansi",		  
			...
			/* Click event handler */
			  ...
			  $("#example tbody").click(function(event) {
					  ...
					  anSelectedApproval1 = element[1];
					  anSelectedApproval2 = element[2];
			  });
			  
			  $('#btnAdd').on('click', function () {
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/permohonan_jadwal_proyek_garansi_add/?reqMode=insert");
			   ...
			   $('#btnEdit').on('click', function () {
				  ...
				  if(anSelectedApproval1 === 'Y' || anSelectedApproval2 === 'Y' || anSelectedApproval1 === 'P' || anSelectedApproval2 === 'P')
				  	  $.messager.alert('Info', "Permohonan telah diverifikasi.", 'info');
				  else
					  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/permohonan_jadwal_proyek_garansi_add/?reqId="+anSelectedId+"&reqMode=update");
			  ...
			  $('#btnLihat').on('click', function () {
				  ...
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/permohonan_jadwal_proyek_garansi_add/?reqId="+anSelectedId+"&reqMode=view");
						...
						if (r){
							$.getJSON("<?=base_url()?>permohonan_proyek_json/delete/"+anSelectedId,
							  function(data){
									  ...
									  oTable.fnReloadAjax("<?=base_url()?>permohonan_proyek_json/json_garansi/");
							});
			  ...
			  $('#btnPosting').on('click', function () {
				...
				$.messager.confirm('Konfirmasi',"Posting proyek garansi terpilih ?",function(r){
					if (r){
						$.getJSON("<?=base_url()?>permohonan_proyek_json/posting_garansi/?reqId="+anSelectedId,
						function(data){
							$.messager.alert('Info', data.PESAN, 'info');
							oTable.fnReloadAjax("<?=base_url()?>permohonan_proyek_json/json_garansi/");
						});
...
<div id="wadah">
    <div class="judul-halaman">Data Proyek Garansi</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
            ...
            <li>
            	<a href="#" id="btnPosting" title="Posting"><img src="<?=base_url()?>images/icon-posting.png" /> Posting</a>
            </li>
        </ul>
    </div>
    <!--
