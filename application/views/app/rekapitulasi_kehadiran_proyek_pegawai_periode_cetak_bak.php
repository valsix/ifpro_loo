...
$statement = " AND A.PERMOHONAN_PROYEK_ID = '".$reqPermohonanProyekId."' AND E.PERIODE = '".$reqPeriode."' ";
...
$permohonan_proyek_cek->selectByParams(array("A.PERMOHONAN_PROYEK_ID" => $reqPermohonanProyekId), -1, -1);
...
$worksheet->set_column(41, 41, 16.00);
$worksheet->set_column(42, 42, 14.00);
...
$worksheet->write(5, 5, "BULAN ".strtoupper(getNamePeriode($reqPeriode))."", $text_format_merge_line_bold);
...
$worksheet->write(5, 41, "TOTAL TIDAK HADIR", $text_format_line_bold_no_bottom);
$worksheet->write(6, 41, "", $text_format_line_bold_no_top);
$worksheet->write(5, 42, "TOTAL KEHADIRAN", $text_format_line_bold_no_bottom);
...
while($permohonan_proyek->nextRow())
{
	...
	$worksheet->write($row, 36, $permohonan_proyek->getField('SAKIT'), $text_format_line_left);
	$worksheet->write($row, 37, $permohonan_proyek->getField('IJIN'), $text_format_line_left);
	$worksheet->write($row, 38, $permohonan_proyek->getField('ALPHA'), $text_format_line_left);
	$worksheet->write($row, 39, $permohonan_proyek->getField('CUTI'), $text_format_line_left);
	$worksheet->write($row, 40, $permohonan_proyek->getField('DINAS'), $text_format_line_left);
	$worksheet->write($row, 41, $permohonan_proyek->getField('TIDAK_HADIR'), $text_format_line_left);
	$worksheet->write($row, 42, $permohonan_proyek->getField('HADIR'), $text_format_line_left);
	$no++;
	$row++;
}
