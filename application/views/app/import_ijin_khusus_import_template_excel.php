...
$fname = tempnam("/tmp", "import_ijin_khusus.xls");
$workbook = & new writeexcel_workbookbig($fname);
$workbook = &new writeexcel_workbook($fname);

$arrayNama[0]="IMPORT DATA";
$arrayNama[1]="IJIN_KOREKSI";
...
for ($i=0; $i < 3; $i++)
{
	...
	if ($i==0)
	{
		...
		$worksheet->set_column(8, 8, 14.00);
			...
			$worksheet->write(0, 2, "TANGGAL AWAL (dd-mm-yyyy)", $text_format_line_bold);
			$worksheet->write(0, 3, "TANGGAL AKHIR (dd-mm-yyyy)", $text_format_line_bold);
			$worksheet->write(0, 4, "TAMBAH HARI SEBELUM (1/0)", $text_format_line_bold);
			$worksheet->write(0, 5, "TAMBAH HARI SESUDAH (1/0)", $text_format_line_bold);
			$worksheet->write(0, 6, "LAMA CUTI", $text_format_line_bold);
			$worksheet->write(0, 7, "IJIN KOREKSI ID", $text_format_line_bold);
		...
		for($j=0; $j<100; $j++)
		{
			...
			$worksheet->write($row, 2, "", $tanggal);
			$worksheet->write($row, 3, "", $tanggal);
	...
	elseif($i==1)
	{
		...
		$ijin_koreksi->selectByParams(array("JENIS" => "IJIN_KHUSUS"));
		
		$worksheet->set_column(0, 0, 15.00);
		$worksheet->set_column(1, 1, 65.00);
		
			$worksheet->write(0, 0, "IJIN KOREKSI ID", $text_format_line_bold);
			$worksheet->write(0, 1, "NAMA IJIN", $text_format_line_bold);
			
		$row=1;
		while($ijin_koreksi->nextRow())
		{
			$worksheet->write($row, 0, $ijin_koreksi->getField("IJIN_KOREKSI_ID"), $text_format);
			$worksheet->write($row, 1, $ijin_koreksi->getField("NAMA"), $text_format);
...
header("Content-Type: application/x-msexcel; name=\"import_ijin_khusus_import_template.xls\"");
header("Content-Disposition: inline; filename=\"import_ijin_khusus_import_template.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>