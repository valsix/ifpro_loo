...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>rekapitulasi_terlambat_pulang_cepat_json/json/?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>&reqDepartemen=<?=$reqDepartemen?>",			  
			...
			/* Click event handler */
			  ...
			  $("#reqBulan, #reqTahun").change(function() { 
				 oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_terlambat_pulang_cepat_json/json/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'));				 	 
			  });
			  
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_terlambat_pulang_cepat_json/json/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+param.id);				 	 
					}
			  });
			  ...
			  $('#btnEkspor').on('click', function () {
				  newWindow = window.open('rekapitulasi_terlambat_pulang_cepat_cetak.php?reqBulan='+$("#reqBulan").val()+'&reqTahun='+ $("#reqTahun").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'), 'Cetak');
				  newWindow.focus();
			  });			  
...
<div id="wadah">
    <div class="judul-halaman">Monitoring Terlambat &amp; Pulang Cepat</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
			<li>
                <a href="#" id="btnEkspor" title="Export Excel"> Export Excel</a>      
            </li>        
        </ul>
    </div>
