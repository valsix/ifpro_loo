...
$fname = tempnam("/tmp", "import_cuti_tahunan.xls");
...
for ($i=0; $i < 2; $i++)
{
	...
	if ($i==0)
	{
		...
		$worksheet->set_column(6, 6, 14.00);
			...
			$worksheet->write(0, 4, "LAMA CUTI", $text_format_line_bold);
			$worksheet->write(0, 5, "MENGHABISKAN CUTI TAHUN", $text_format_line_bold);
...
header("Content-Type: application/x-msexcel; name=\"import_cuti_tahunan_import_template.xls\"");
header("Content-Disposition: inline; filename=\"import_cuti_tahunan_import_template.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>