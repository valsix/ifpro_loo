...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>daily_report_json/json",		  
			...
			/* Click event handler */
			  ...
			  $('#btnAdd').on('click', function () {
				  document.location.href = "permohonan_cuti_tahunan_add";
			  ...
			  $('#btnEdit').on('click', function () {
				...
				if(anSelectedPublish == "" || anSelectedPublish == "0")	  
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/permohonan_cuti_tahunan_add/?reqId="+anSelectedId+"&reqMode=update");
				else
			  ...
			  $('#btnLihat').on('click', function () {
				  ...
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/permohonan_cuti_tahunan_add/?reqId="+anSelectedId+"&reqMode=view");
			   ...
			   $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("<?=base_url()?>permohonan_cuti_tahunan_json/delete/?reqId="+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("<?=base_url()?>permohonan_cuti_tahunan_json/json/?reqStatus=" +$("#reqStatus").val());
							});
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>daily_report_json/json/?reqDepartemen="+param.id+"&reqTanggal="+$('#reqTanggal').datebox('getValue'));
					}
			  });
			  ...
			  $('#reqTanggal').datebox({
				onChange:function(newValue,oldValue){
					...
					{
						val_tanggal = newValue;
						oTable.fnReloadAjax("<?=base_url()?>daily_report_json/json?reqDepartemen="+$('#reqDepartemen').combotree('getValue')+"&reqTanggal="+ val_tanggal);
					}
					else
					{
						val_tanggal = "";
...
<div id="wadah">
    <div class="judul-halaman">Rekapitulasi Daily Activity</div>
    ...
    <div id="parameter-tambahan">
		...
		?>
			Tanggal : <input id="reqTanggal" class="easyui-datebox" name="reqTanggal" value="<?=$reqTanggal?>">
    </div>
    ...
    <thead>
        <tr>
            ...
            <th width="50px">Jam Awal</th>
            <th width="50px">Jam Akhir</th>
            <th width="250px">Nama Kegiatan</th>
            <th width="50px">Tanggal Selesai</th>
            <th width="50px">Jam Selesai</th>
