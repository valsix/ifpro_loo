...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>sinkronisasi_pegawai_json/json_pihak_ketiga/?reqDepartemen=<?=$reqDepartemen?>",		  
			...
			/* Click event handler */
			  ...
			  /*$('#btnAdd').on('click', function () {
				  document.location.href = "app/index/pegawai_pihak_ketiga_add";
			  ...
			  $('#btnEdit').on('click', function () {
				  ...
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/pegawai_pihak_ketiga_add/?reqId="+anSelectedId+"&reqMode=view");
			   ...
			   $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("<?=base_url()?>sinkronisasi_pegawai_json/delete_pihak_ketiga/?reqId="+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("<?=base_url()?>sinkronisasi_pegawai_json/json_pihak_ketiga/?reqDepartemen=" +$("#reqDepartemen").combotree('getValues'));
							});
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>sinkronisasi_pegawai_json/json_pihak_ketiga/?reqDepartemen="+param.id);	
					}
			  });
			  
			  $('#btnImport').on('click', function () {
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/pegawai_pihak_ketiga_import/");
...
<div id="wadah">
    <div class="judul-halaman">Pihak Ketiga</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
			<li>
                ...
                <span><a href="#" id="btnEditData" title="Ubah"><img src="<?=base_url()?>images/icon-edit.png" /> Ubah</a> </span>
                <span><a href="#" id="btnDelete" title="Hapus"><img src="<?=base_url()?>images/icon-hapus.png" /> Hapus</a> </span>
                <!--<span><a href="#" id="btnEdit" title="Ubah Finger"><img src="<?=base_url()?>images/icon-edit.png" /> Ubah Finger</a> </span>-->
            </li>
            <li>
            	<span><a href="#" id="btnImport" title="Import"> Import</a></span>
            </li>    
            <!--<li>
            	<a href="#" id="btnSinkronisasi" title="Sinkronisasi"><img src="<?=base_url()?>images/icon-sinkronisasi.png" /> Sinkronisasi</a>
            </li>-->    
        </ul>
    </div>
