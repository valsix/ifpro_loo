...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>jkk_jks_periode_json/json",			  
			...
			/* Click event handler */
			  ...
			  $("#example tbody").click(function(event) {
					  ...
					  anSelectedSemester = element[1];
					  anSelectedTahun = element[2];
			  });
			  
			  $('#btnAdd').on('click', function () {
				  //window.parent.openPopup("<?=base_url()?>app/loadUrl/app/jkk_jks_periode_add");
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/jkk_jks_periode_add");
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/jkk_jks_periode_add/?reqSemester="+anSelectedSemester+"&reqTahun="+anSelectedTahun);
			  ...
			  $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("<?=base_url()?>jkk_jks_periode_json/delete/?reqSemester="+anSelectedSemester+"&reqTahun="+anSelectedTahun,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("<?=base_url()?>jkk_jks_periode_json/json");
							});
...
<div id="wadah">
    <div class="judul-halaman">Data Master JKK/JKS Periode</div>
