...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>permohonan_proyek_json/json_verifikator_anggota",		  
			...
			/* Click event handler */
			  ...
			  $('#btnAdd').on('click', function () {
			   ...
			   $('#btnEdit').on('click', function () {
				  ...
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/permohonan_verifikasi_jadwal_proyek_pegawai_add/?reqId="+anSelectedId+"&reqMode=update");
			  ...
			  $('#btnLihat').on('click', function () {
				  ...
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/permohonan_verifikasi_jadwal_proyek_pegawai_add/?reqId="+anSelectedId+"&reqMode=viewuser");
			...
			$('#btnPosting').on('click', function () {
				...
				$.messager.confirm('Konfirmasi',"Posting daftar anggota proyek ?",function(r){
					if (r){
						$.getJSON("<?=base_url()?>kelompok_proyek_pegawai_json/posting/?reqId="+anSelectedId,
						function(data){
							$.messager.alert('Info', data.PESAN, 'info');
							oTable.fnReloadAjax("<?=base_url()?>permohonan_proyek_json/json_user_resplan/");
						});
					}
				});	
			});
			  ...
			  $("#reqStatus").change(function() { 			  
				 oTable.fnReloadAjax("<?=base_url()?>permohonan_proyek_json/json_verifikator_anggota/?reqStatus="+$("#reqStatus").val());				 	 
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Permohonan Data Anggota Proyek</div>
