...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>import_penangguhan_cuti_json/json?reqDepartemen=<?=$reqDepartemen?>",		  
			...
			/* Click event handler */
			  ...
			  $('#btnAdd').on('click', function () {
			   ...
			   $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("<?=base_url()?>import_penangguhan_cuti_json/delete/"+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("<?=base_url()?>import_penangguhan_cuti_json/json?reqDepartemen=<?=$reqDepartemen?>");
							});
			  ...
			  $('#btnImport').on('click', function () {
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/import_penangguhan_cuti_import/");
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>import_penangguhan_cuti_json/json?reqDepartemen="+param.id);	
					}
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Import Penangguhan Cuti</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
			<li>
                <a href="#" id="btnImport" title="Import"><img src="<?=base_url()?>images/icon-tambah.png" /> Import</a> 
    ...
    <thead>
        <tr>
            ...
            <th width="70px">Lama Penangguhan</th>
        </tr>
    </thead>
    </table> 
</div>
