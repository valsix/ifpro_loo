...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>rekapitulasi_json/json_potongan_absensi/?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>&reqDepartemen=<?=$reqDepartemen?>",			  
			...
			/* Click event handler */
			  ...
			  $('#btnProses').on('click', function () {
				  if(confirm("Proses keterlambatan ?"))
				  {
					  var win = $.messager.progress({
						  title:'Proses data keterlambatan.',
						  msg:'Proses data...'
					  });									
					  var jqxhr = $.get("<?=base_url()?>rekapitulasi_json/proses_rekap_terlambat/?reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'), function(data) {
						  
						  oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_json/json_potongan_absensi/?reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'));	
			  ...
			  $('#btnDetil').on('click', function () {
				  if(anSelectedData == "")
					  return false;	
					  		
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/rekapitulasi_potong_absensi_detil/?reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqPegawaiId="+anSelectedId);
			 ...
			 $("#reqTahun").change(function() {
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>rekapitulasi_json/json_potongan_absensi/?reqTahun=" +$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+param.id);
					}
			  });
			  ...
			  $('#btnCetak').on('click', function () {
				  newWindow = window.open('rekapitulasi_potong_absensi_cetak.php?reqBulan='+$("#reqBulan").val()+'&reqTahun='+ $("#reqTahun").val()+"&reqDepartemen="+$('#reqDepartemen').combotree('getValue'), 'Cetak');
				  newWindow.focus();
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Rekapitulasi Potongan Absensi</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
			<li>
                <a href="#" id="btnProses" title="Proses"> Proses</a>     
            </li>  
			<li>
                <a href="#" id="btnCetak" title="Cetak"> Cetak</a>     
            </li>
            <li>
                <a href="#" id="btnDetil" title="Detil">Detil</a>   
            </li>
        </ul>
    </div>
    ...
    <thead>
            <tr>
                ...
                <th width="250px" style="text-align:center">Nama</th> 
                <th width="70px" style="text-align:center">Status Karyawan</th> 
                <th width="70px" style="text-align:center">Finger ID</th> 
                <th width="70px" style="text-align:center">Jumlah Menit Terlambat</th>
                <th width="70px" style="text-align:center">Konversi Jam</th>
            </tr>
    </thead>
    </table> 
</div>
