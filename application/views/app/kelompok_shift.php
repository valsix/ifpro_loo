    ...
    $(document).ready( function () {
		...
		$(window).resize(function() {
         ...
         "aoColumns": [
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>kelompok_shift_json/json",		
			...
			/* Click event handler */
			  ...
			  $('#btnAdd').on('click', function () {
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/kelompok_shift_add");
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/kelompok_shift_add/"+anSelectedId);
			  ...
			  $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("<?=base_url()?>kelompok_shift_json/delete/"+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("<?=base_url()?>kelompok_shift_json/json");
							});
...
<div id="wadah">
    <div class="judul-halaman">Data <?=$judulnya?> <?=$namacab?></div>
    ...
    <thead>
        <tr>
            ...
            <th width="200px">Nomor Permohonan Shift</th>
