...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>pegawai_json/json_cabang",			  
			...
			/* Click event handler */
			  ...
			  $("#example tbody").click(function(event) {
					  ...
					  anSelectedJabatan = element[13];
					  anSelectedRow = event.target.parentNode;
			  });
			  
			  $('#btnAdd').on('click', function () {
				  window.parent.createWindow2("<?=base_url()?>app/loadUrl/app/tiket_add");
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.createWindow2("<?=base_url()?>app/loadUrl/app/tiket_add/?reqId="+anSelectedId);
			  ...
			  $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("<?=base_url()?>tiket_json/delete/?reqId="+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("<?=base_url()?>tiket_json/json");
							});
			  ...
			  $('#btnPilih').on('click', function () {
				  anSelectedRow.remove();
				  if(anSelectedData == "")
					  return false;				
					
				 parent.createRow(anSelectedId,anSelectedNama,anSelectedJabatan);
...
<div id="wadah">
    <div class="judul-halaman">Data Pegawai</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
			<li>
                <a href="#" id="btnPilih" title="Pilih"><img src="<?=base_url()?>images/icon-tambah.png" /> Tambah</a>      
            </li>        
           <!-- <li>
