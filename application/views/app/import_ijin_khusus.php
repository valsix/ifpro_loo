...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>import_ijin_khusus_json/json?reqDepartemen=<?=$reqDepartemen?>",		  
			...
			/* Click event handler */
			  ...
			  $('#btnAdd').on('click', function () {
				  document.location.href = "app/index/import_ijin_khusus_add";
			   ...
			   $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;			
				  document.location.href = "app/index/import_ijin_khusus_add/reqMode=view"+anSelectedId;	
			   ...
			   $('#btnDelete').on('click', function () {
					...
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("import_cuti_tahunan_json/delete/"+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("import_cuti_tahunan_json/json/?reqStatusAgenda=" +$("#reqStatusAgenda").val());
							});
			  ...
			  $('#btnImport').on('click', function () {
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/import_ijin_khusus_import/");
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						oTable.fnReloadAjax("<?=base_url()?>import_ijin_khusus_json/json?reqDepartemen="+param.id);	
					}
			  });
		});
</script>
...
<div id="wadah">
    <div class="judul-halaman">Import Ijin, Ijin Khusus</div>
