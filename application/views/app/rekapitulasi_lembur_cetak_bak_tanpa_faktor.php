...
$worksheet->set_column(4, 4, 26.00);
...
$worksheet->write(5, 36, "JAM LEMBUR", $text_format_line_bold_no_bottom);
...
while($absensi_rekap->nextRow())
{
	...
	// $worksheet->write($row, 4, $absensi_rekap->getField('STATUS_PEGAWAI'), $text_format_line_left);
		...
		for ($k=5; $k<=35; $k++)
		{
			$arrData = $absensi_rekap->getField('TGL_'.($k-4));
			$worksheet->write($row, $k, $arrData, $text_format_line);
			$arrTotal += $arrData;
		}
		
	$worksheet->write($row, 36, $arrTotal, $text_format_line);
