...
if(varItem == 'permohonan_jadwal_proyek_garansi_add_data'){
	$("#permohonan_jadwal_proyek_garansi_add_data").addClass("aktif");
	$('#permohonan_jadwal_proyek_garansi_add_data').css({'background-position': '0 -27px'});
	mainFramePop.location.href='app/loadUrl/app/permohonan_jadwal_proyek_garansi_add_data/?reqId=<?=$reqId?>&reqMode=<?=$reqMode?>';
	document.getElementById('trdetil').style.display = 'none';	
}
else if(varItem == 'permohonan_jadwal_proyek_garansi_add_anggota'){
	$("#permohonan_jadwal_proyek_garansi_add_anggota").addClass("aktif");
	$('#permohonan_jadwal_proyek_garansi_add_anggota').css({'background-position': '0 -27px'});
	mainFramePop.location.href='app/loadUrl/app/permohonan_jadwal_proyek_garansi_add_anggota/?reqId=<?=$reqId?>&reqMode=<?=$reqMode?>';
	document.getElementById('trdetil').style.display = 'none';	
}
return true;
}
	...
	<div id="wrapper" class="bg-kiri-popup" style="overflow:hidden;">
        ...
        <nav class="navbar navbar-default navbar-static-top popup-atas" role="navigation" style="margin-bottom: 0;">
            <div class="navbar-header navbar-header-popup">
                ...
                <a class="navbar-brand">Data Garansi Proyek</a>
            </div>
            <!-- /.navbar-header -->
                ...
                <div class="sidebar-nav navbar-collapse">
                    <ul class="nav" id="side-menu">
                        <li style="border:none;">
                            ...
                            <div id="menu-kiri">
                                <div><a id="permohonan_jadwal_proyek_garansi_add_data" onClick="executeOnClick('permohonan_jadwal_proyek_garansi_add_data');" class="aktif">Data Garansi Proyek</a></div>                                
                                <?
								if($reqId == "")
								{}
								else
								{
                                ?>
                                <div><a id="permohonan_jadwal_proyek_garansi_add_anggota" onClick="executeOnClick('permohonan_jadwal_proyek_garansi_add_anggota');">Anggota Garansi Proyek</a></div>
                                <?
								}
                                ?>
                            </div>
        ...
        <div class="konten-utama-popup">
        	<table cellpadding="0" cellspacing="0" style="width:100%; height:100%;">
                <tr height="50%">
                    <td><iframe src="app/loadUrl/app/permohonan_jadwal_proyek_garansi_add_data/?reqId=<?=$reqId?>&reqMode=<?=$reqMode?>" class="mainframe" id="idMainFrame" name="mainFramePop" width="100%" height="100%" scrolling="auto" frameborder="0" style="display:block;"></iframe></td>
                </tr>
