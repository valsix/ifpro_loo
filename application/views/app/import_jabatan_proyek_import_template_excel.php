...
$fname = tempnam("/tmp", "import_jabatan_proyek.xls");
...
for ($i=0; $i < 1; $i++)
{
	...
	if ($i==0)
	{
		...
		$worksheet->set_column(4, 4, 14.00);
			...
			$worksheet->write(0, 0, "NAMA JABATAN", $text_format_line_bold);
...
header("Content-Type: application/x-msexcel; name=\"import_jabatan_proyek_template.xls\"");
header("Content-Disposition: inline; filename=\"import_jabatan_proyek_template.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>