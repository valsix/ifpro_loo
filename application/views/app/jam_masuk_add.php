...
$this->load->model('JamMasuk');

$jam_masuk = new JamMasuk();
...
{
	$reqMode = "update";	
	$jam_masuk->selectByParams(array('JAM_MASUK_ID'=>$reqId), -1, -1);
	$jam_masuk->firstRow();
	
	$tempNama= $jam_masuk->getField('NAMA');
	$tempKeterangan= $jam_masuk->getField('KETERANGAN');
	$tempToleransiTerlambat= $jam_masuk->getField('TOLERANSI_TERLAMBAT');
	$tempToleransiAmbilDataAwal= $jam_masuk->getField('TOLERANSI_AMBIL_DATA_AWAL');
	$tempToleransiAmbilDataAkhir= $jam_masuk->getField('TOLERANSI_AMBIL_DATA_AKHIR');
}
...
$(function(){
	$('#ff').form({
		url:'<?=base_url()?>jam_masuk_json/add',
		onSubmit:function(){
