...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "<?=base_url()?>pelaksana_harian_json/json",			  
			...
			/* Click event handler */
			  ...
			  $("#example tbody").click(function(event) {
					  ...
					  anSelectedStatusAktif = element[1];
					  anSelectedPegawaiId = element[2];
					  anSelectedPegawaiPhId = element[3];
			  });
			  
			  $('#btnAdd').on('click', function () {
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/pelaksana_harian_add");
			  ...
			  $('#btnEdit').on('click', function () {
				  ...
				  if(anSelectedStatusAktif == "1")
				  {
					  $.messager.alert('Info', "Pelaksana harian sudah diaktifkan, data tidak dapat diubah.", 'info');
					  return false;
				  }
				  
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/pelaksana_harian_add/?reqId="+anSelectedId);
			  ...
			  $('#btnNonAktif').on('click', function () {
				  ...
				  if(anSelectedStatusAktif == "0")
				  {
					  $.messager.alert('Info', "Pelaksana harian sudah non-aktif.", 'info');
					  return false;
				  }
				  
				  if(confirm("Non-aktifkan pelaksana harian ?"))
				  {
					  var win = $.messager.progress({
						  title:'Proses non-aktif data.',
						  msg:'Proses data...'
					  });									
					  var jqxhr = $.get("<?=base_url()?>pelaksana_harian_json/pengalihan_non_aktif/?reqId="+anSelectedId, function(data) {						  
			  ...
			  $('#btnAktif').on('click', function () {
				  ...
				  if(anSelectedStatusAktif == "1")
				  {
					  $.messager.alert('Info', "Pelaksana harian sudah diaktifkan.", 'info');
					  return false;
				  }
					  
				  window.parent.openPopup("<?=base_url()?>app/loadUrl/app/pelaksana_harian_aktif/?reqId="+anSelectedId+"&reqPegawaiId="+anSelectedPegawaiId+"&reqPegawaiPhId="+anSelectedPegawaiPhId);
			  ...
			  $('#btnDelete').on('click', function () {
				  ...
				  if(anSelectedStatusAktif == "1")
				  {
					  $.messager.alert('Info', "Pelaksana harian sudah diaktifkan, data tidak dapat dihapus.", 'info');
					  return false;
				  }
				  						  
					$.messager.confirm('Konfirmasi',"Hapus data terpilih?",function(r){
						if (r){
							$.getJSON("<?=base_url()?>pelaksana_harian_json/delete/"+anSelectedId,
							  function(data){
									  $.messager.alert('Info', data.PESAN, 'info');
									  oTable.fnReloadAjax("<?=base_url()?>pelaksana_harian_json/json");
							});
...
<div id="wadah">
    <div class="judul-halaman">Data Master Pelaksana Harian (PH)</div>
    <div id="bluemenu" class="bluetabs" style="background:url(<?=base_url()?>css/media/bluetab.gif)">    
        <ul>
            ...
            <li>
                <a href="#" id="btnAktif" title="Aktifkan"><img src="<?=base_url()?>images/icon-posting.png" /> Aktifkan</a>   
                <a href="#" id="btnNonAktif" title="Non-Aktif"><img src="<?=base_url()?>images/icon-sinkronisasi.png" /> Non-Aktifkan</a>            
            </li>
        </ul>
    </div>
    ...
    <thead>
        <tr>
            ...
            <th>Id</th>
            <th>Id</th>
            <th width="200px">Nama Pegawai Diganti</th>
            <th width="200px">Nama Pegawai Pengganti</th>
            <th width="80px">Tanggal Awal</th> 
            <th width="80px">Tanggal Akhir</th>
