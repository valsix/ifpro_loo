...
$(function(){
	$('#ff').form({
		url:'<?=base_url()?>permohonan_proyek_json/approval_garansi',
		onSubmit:function(){
			var f = this;
...
function create(idKelompok)
{
	nomor += 1;
	$(function () {
		$.get("<?=base_url()?>app/loadUrl/app/permohonan_jadwal_proyek_garansi_add_anggota_template/?reqId="+idKelompok+"&reqNomor="+nomor, function (data) {
			$("#tbData").append(data);
		});
	});
}
...
<body class="bg-kanan-full" onload="AlasanTolak()">
	<div id="judul-popup">Data Proyek Garansi</div>
	<div id="konten">
    	<div id="popup-tabel2">
            ...
            <table class="table">
                    <thead>
                        ...
                        <tr>
                        	<th colspan="3">Anggota Proyek</th>
                        </tr>
                        <tr>
                        	<td colspan="3">
                                <table class="table">
                                    <thead>
                                        <tr>
                                            <th style="text-align:center; width:20%">NIP <a style="cursor:pointer" id="btnAdd" title="Tambah" onClick="openPopupProyek('<?=base_url()?>app/loadUrl/main/pegawai_proyek_anggota_garansi/?reqId=<?=$reqId?>&reqPermohonanProyekParentId=<?=$reqPermohonanProyekParentId?>')"><img src="<?=base_url()?>images/icon-tambah.png" /></a></th>
                                    ...
                                    ?>
                                        <tr id="<?=$i?>">
                                            ...
                                            <td align="center">
												...
												$reqPegawaiIdApproval1 = $pegawai_spv->getField("PEGAWAI_ID");
												
												$pegawai_manager = new Pegawai();
												$pegawai_manager->selectByParams(array("A.JABATAN_ID" => 'KP00136D'));
												$pegawai_manager->firstRow();
												
												$reqPegawaiIdApproval2 = $pegawai_manager->getField("PEGAWAI_ID");
												?>
												<input type="hidden" name="reqPegawaiIdApproval1" id="reqPegawaiIdApproval1" value="<?=$reqPegawaiIdApproval1?>" />
                                                <input type="hidden" name="reqPegawaiIdApproval2" id="reqPegawaiIdApproval2" value="<?=$reqPegawaiIdApproval2?>" />
                                                
                                                <input type="hidden" name="reqJabatanProyekId[]" value="<?=$kelompok_proyek_pegawai->getField("JABATAN_PROYEK_ID")?>">
