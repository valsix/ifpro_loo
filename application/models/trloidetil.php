...
***************************************************************************************************** */
  ...
  ***/
  ...
  class TrLoiDetil extends Entity{ 
    ...
    **/
	...
	function insert()
	{
		$this->setField("TR_LOI_DETIL_ID", $this->getNextId("TR_LOI_DETIL_ID","tr_loi_detil")); 
		$str = "
		...
		$this->id = $this->getField("TR_LOI_DETIL_ID");
		$this->query = $str;
		// echo $str;exit;
  	...
  	function update()
	{
		...
		WHERE TR_LOI_DETIL_ID= ".$this->getField("TR_LOI_DETIL_ID")."
		"; 
		// echo $str;exit;
		$this->query = $str;
		return $this->execQuery($str);
    ...
    **/ 
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1,$statement="", $order=" ORDER BY A.TR_LOI_DETIL_ID ASC")
	{
		$str = "
		SELECT 
		A.*
    ...
    **/ 
	...
	{
		$str = "SELECT COUNT(1) AS ROWCOUNT FROM tr_loi_detil A WHERE 1 = 1 ".$statement; 
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key = '$val' ";
		}
