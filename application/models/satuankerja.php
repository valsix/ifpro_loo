...
class SatuanKerja extends Entity
{
	...
	function insertJabatanStruktural()
	{
		...
		(
			...
			, KELOMPOK_JABATAN, LAST_CREATE_USER, LAST_CREATE_DATE 
		) 
		VALUES
		(
			'" . $nextSatKerId . "'
	...
	function updateJabatanStruktural()
	{
		...
		UPDATE SATUAN_KERJA SET
			...
			, APPROVAL_STTPD= '" . $this->getField("APPROVAL_STTPD") . "'
	...
	function selectByParams($paramsArray=array(),$limit=-1,$from=-1,$statement="", $order=" ORDER BY SATUAN_KERJA_ID ASC")
	{
		$str = "
				SELECT 
					...
					, UNIT_KERJA_NAMA
					, APPROVAL_STTPD, APPROVAL_STTPD_NAMA
    ...
    function selectsatuankerjatree($paramsArray = array(), $limit = -1, $from = -1, $nip="", $statement = "", $order = "ORDER BY DEPTH DESC")
	{
		...
		WITH RECURSIVE PARENTS AS (
			SELECT
				SATUAN_KERJA_ID, TREE_ID, TREE_PARENT, NAMA, NAMA_PEGAWAI
				, CASE WHEN COALESCE(NULLIF(NIP, ''), NULL) IS NULL THEN APPROVAL_STTPD ELSE NIP END NIP
				, JABATAN, KELOMPOK_JABATAN, APPROVAL_STTPD, APPROVAL_STTPD_NAMA
				, 0 AS DEPTH
			...
			UNION
			SELECT
				OP.SATUAN_KERJA_ID, OP.TREE_ID, OP.TREE_PARENT, OP.NAMA, OP.NAMA_PEGAWAI
				, CASE WHEN COALESCE(NULLIF(OP.NIP, ''), NULL) IS NULL THEN OP.APPROVAL_STTPD ELSE OP.NIP END NIP
				, OP.JABATAN, OP.KELOMPOK_JABATAN, OP.APPROVAL_STTPD, OP.APPROVAL_STTPD_NAMA
				, DEPTH - 1
	...
	function selectdata($paramsArray=array(),$limit=-1,$from=-1,$statement="", $order=" ORDER BY A.SATUAN_KERJA_ID ASC")
	{
		$str = "
		SELECT 
		A.*
    ...
    function selectuseratasan($statement="")
	{
		$str = "
		SELECT
			DISTINCT A.USER_ATASAN_ID
		FROM
		(
			SELECT
				DISTINCT A.USER_ATASAN_ID
			...
			UNION ALL
			SELECT
				DISTINCT A.NIP_ATASAN_MUTASI USER_ATASAN_ID
			FROM SURAT_MASUK A
			WHERE 1=1
			".$statement."
		) A
		WHERE COALESCE(NULLIF(A.USER_ATASAN_ID, ''), NULL) IS NOT NULL
		";
    ...
    function selectByParamsKelompok($statement="")
		{
		$str = "
		SELECT
			*
