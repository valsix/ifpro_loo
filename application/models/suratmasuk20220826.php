...
class SuratMasuk extends Entity
{
	...
	function selectByParams($paramsArray = array(), $limit = -1, $from = -1, $stat = '', $order = "")
	{
		...
		, A.DASAR
		, A.ISI_PERINTAH
		, A.LAIN_LAIN, A.DARI_INFO, A.NOMOR_SURAT_INFO, A.STATUS_APPROVE
		FROM SURAT_MASUK A
	...
	function selectByParamsSurat($paramsArray = array(), $limit = -1, $from = -1, $stat = '', $order = "")
	{
		...
		SELECT 
			...
			SURAT_PDF
			, AMBIL_SURAT_MASUK_TUJUAN(A.SURAT_MASUK_ID, 'TEMBUSAN') TEMBUSAN
			, AMBIL_SURAT_MASUK_TUJUAN_ASC_PARAM(A.SURAT_MASUK_ID, 'TEMBUSAN', 'xxxx') TEMBUSAN_PARAM
	...
	function selectByParamsNewSuratMasuk($paramsArray = array(), $limit = -1, $from = -1, $userid, $cabangid, $kelompokjabatan, $stat = '', $order = "ORDER BY TANGGAL_ENTRI DESC")
	{
		$str = "
		SELECT
			A.SURAT_MASUK_ID
	...
	function getCountByParamsNewSuratMasuk($paramsArray = array(), $userid, $cabangid, $kelompokjabatan, $statement = "")
	{
		$str = "
		SELECT COUNT(1) AS ROWCOUNT 
		FROM SURAT_MASUK A
	...
	function selectByParamsSuratKeluar1($paramsArray = array(), $limit = -1, $from = -1, $userid, $stat = '', $order = "")
	{
		...
		INNER JOIN (
			...
			(
				...
				(
					(
						A.USER_ATASAN_ID IN (".$userid.") --AND COALESCE(NULLIF(A.NIP_ATASAN_MUTASI, ''), NULL) IS NULL
					)
				)
				UNION ALL
				SELECT
