...
class SuratMasuk extends Entity
{
	...
	function selectByParamsStatus($paramsArray = array(), $limit = -1, $from = -1, $userid, $stat = '', $order = "")
	{
		...
		FROM surat_masuk A
		LEFT JOIN (SELECT * FROM t_surat_info('1')) S ON A.SURAT_MASUK_ID = S.SURAT_MASUK_ID
		LEFT JOIN (SELECT * FROM p_surat_status('".$userid."')) SM ON A.SURAT_MASUK_ID = SM.SM_ID
