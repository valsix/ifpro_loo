  ...
  ***/
  ...
  class PermohonanStpd extends Entity{ 
    ...
    **/
    ...
    function updatestatusrealisasitanpafile()
	{
		$str = "
		UPDATE PERMOHONAN_STPD
		SET
    ...
    function updatesetujui()
	{
		...
		UPDATE PERMOHONAN_STPD
		SET
			APPROVAL_QR_DATE = CURRENT_TIMESTAMP
			, STATUS_SURAT= 'SETUJU'
			, KET_REALISASI= '".$this->getField("KET_REALISASI")."'
			, TOTAL_REALISASI = 
			(
				SELECT COALESCE((SELECT SUM(REALISASI) FROM PERMOHONAN_STPD_BIAYA_DINAS PS WHERE PERMOHONAN_STPD_ID = ".$this->getField("PERMOHONAN_STPD_ID")."),0)
			)
			, DOKUMEN_ACUAN= '".$this->getField("DOKUMEN_ACUAN")."'
			, LOKASI_DINAS= '".$this->getField("LOKASI_DINAS")."'
			, TANGGAL_KEMBALI= ".$this->getField("TANGGAL_KEMBALI")."
    ...
    function updatestatusrealisasipakaifile()
	{
		$str = "
		UPDATE PERMOHONAN_STPD
		SET
    ...
    function updaterealisasitanpafile()
	{
		$str = "
		UPDATE PERMOHONAN_STPD
		SET
    ...
    function updaterealisasipakaifile()
	{
		$str = "
		UPDATE PERMOHONAN_STPD
		SET
    ...
    function updateuserbantuteruskan()
	{
		$str = "
		UPDATE PERMOHONAN_STPD
		SET
			USER_BANTU_APPROVAL_KIRIM= '1'
		WHERE PERMOHONAN_STPD_ID= '".$this->getField("PERMOHONAN_STPD_ID")."'
		AND USER_BANTU_APPROVAL_NIP= '".$this->getField("USER_BANTU_APPROVAL_NIP")."'
		"; 
		$this->query = $str;
		// echo $str;exit;
		return $this->execQuery($str);
    ...
    **/ 
    ...
    {
    	/*,  TO_CHAR((select psl.tanggal from permohonan_stpd_log psl where psl.PERMOHONAN_STPD_id=a.PERMOHONAN_STPD_id and status_surat='KIRIM') ::DATE, 'DD-MM-YYYY') AS tglkirim
    	, TO_CHAR((select psl.tanggal from permohonan_stpd_log psl where psl.PERMOHONAN_STPD_id=a.PERMOHONAN_STPD_id and status_surat='SETUJU') ::DATE, 'DD-MM-YYYY') AS tglsetuju
    	, TO_CHAR((select psl.tanggal from permohonan_stpd_log psl where psl.PERMOHONAN_STPD_id=a.PERMOHONAN_STPD_id and status_surat='SETUJUKIRIM') ::DATE, 'DD-MM-YYYY') AS tglsetujuKirim*/
		$str = "
		SELECT
			...
			, TO_CHAR(A2.TGLKIRIM, 'DD-MM-YYYY') TGLKIRIM
			, TO_CHAR(A2.TGLSETUJU, 'DD-MM-YYYY') TGLSETUJU
			, TO_CHAR(A2.TGLSETUJUKIRIM, 'DD-MM-YYYY') TGLSETUJUKIRIM
			, A.*
		FROM PERMOHONAN_STPD A
		LEFT JOIN
		(
		...
		(
			SELECT 
			PSL.PERMOHONAN_STPD_ID
			, MAX(CASE WHEN STATUS_SURAT = 'KIRIM' THEN PSL.TANGGAL ELSE NULL END)::DATE TGLKIRIM
			, MAX(CASE WHEN STATUS_SURAT = 'SETUJU' THEN PSL.TANGGAL ELSE NULL END)::DATE TGLSETUJU
			, MAX(CASE WHEN STATUS_SURAT = 'SETUJUKIRIM' THEN PSL.TANGGAL ELSE NULL END)::DATE TGLSETUJUKIRIM
			FROM PERMOHONAN_STPD_LOG PSL WHERE 1=1
			GROUP BY PSL.PERMOHONAN_STPD_ID
		) A2 ON A.PERMOHONAN_STPD_ID = A2.PERMOHONAN_STPD_ID
    ...
    function selectrealisasiparaf($paramsArray=array(),$limit=-1,$from=-1, $statement="", $sorder= "ORDER BY A.PERMOHONAN_STPD_ID, A.NO_URUT DESC")
	{
		$str = "
		SELECT 
			A.*, TO_CHAR(last_update_date::DATE, 'DD-MM-YYYY') AS DateUpdate
    ...
    function selectByParamsStatus($paramsArray=array(),$limit=-1,$from=-1, $statement="", $sOrder="")
	{
		$str = "
		SELECT 
			...
			WHEN UPPER(A.STATUS_SURAT) = UPPER('SELESAI') THEN 'Selesai'
			ELSE 'BELUM DI TENTUKAN' END STATUS_NAMA
			,
			CASE
			WHEN 
				COALESCE(NULLIF(A.USER_BANTU_APPROVAL_NIP, ''), NULL) IS NOT NULL AND
				COALESCE(NULLIF(A.USER_BANTU_APPROVAL_KIRIM, ''), NULL) IS NULL
			THEN A.USER_BANTU_APPROVAL_JABATAN || ' - ' || A.USER_BANTU_APPROVAL_NAMA
			ELSE B1.POSISI_SURAT_INFO END POSISI_SURAT_INFO
