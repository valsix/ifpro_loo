...
class SuratMasukParaf extends Entity
{
	...
	function resetparalelnourut()
	{
		$str1= "
		UPDATE SURAT_MASUK_PARAF
		SET NO_URUT = (COALESCE((SELECT MAX(NO_URUT) FROM SURAT_MASUK_PARAF WHERE SURAT_MASUK_ID = ".$this->getField("SURAT_MASUK_ID")." AND COALESCE(NULLIF(KONDISI_PARAF, ''), NULL) IS NULL),0) + 1)
		...
		$str2= "
		UPDATE SURAT_MASUK_PARAF
		SET NO_URUT = (COALESCE((SELECT MAX(NO_URUT) FROM SURAT_MASUK_PARAF WHERE SURAT_MASUK_ID = ".$this->getField("SURAT_MASUK_ID")." AND UPPER(KONDISI_PARAF) = UPPER('PARALEL') AND STATUS_BANTU = 1),0) + 1)
		WHERE SURAT_MASUK_ID = ".$this->getField("SURAT_MASUK_ID")."
		AND UPPER(KONDISI_PARAF) = UPPER('PARALEL')
	...
	function selectByParams($paramsArray = array(), $limit = -1, $from = -1, $statement = "", $order = " ORDER BY SURAT_MASUK_PARAF_ID ASC")
	{
		$str = "
		SELECT 
		SURAT_MASUK_PARAF_ID, SURAT_MASUK_ID, SATUAN_KERJA_ID_TUJUAN, USER_ID, NAMA_USER, NAMA_SATKER, 
		STATUS_PARAF, KODE_PARAF, LAST_CREATE_USER, LAST_CREATE_DATE, 
		LAST_UPDATE_USER, LAST_UPDATE_DATE, NO_URUT, STATUS_BANTU
